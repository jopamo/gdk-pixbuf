...
static gpointer record_bytes(gpointer mem, gsize bytes) {
    if (mem == NULL || (current_allocation + bytes) > max_allocation) {
        if (mem)
            __libc_free(mem);
...
static void mem_test(const gchar* bytes, gsize len) {
...
static void almost_exhaust_memory(void) {
