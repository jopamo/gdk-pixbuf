...
static guint32 get_pixel(GdkPixbuf* pixbuf, int x, int y) {
    guchar* colors;
    guint32 pixel;

    colors = ((guchar*)gdk_pixbuf_get_pixels(pixbuf) + gdk_pixbuf_get_n_channels(pixbuf) * x +
...
static gboolean pixbuf_not_changed_outside_area(GdkPixbuf* pixbuf_new,
    ...
    for (x_curr = 0; x_curr < pixbuf_width; x_curr++) {
        for (y_curr = 0; y_curr < pixbuf_height; y_curr++) {
            if ((x_curr >= x) && (x_curr < x + width) && (y_curr >= y) && (y_curr < y + height)) {
            ...
            if (get_pixel(pixbuf_new, x_curr, y_curr) != get_pixel(pixbuf_old, x_curr, y_curr)) {
                printf("Pixel at (%d, %d) changed from %x to %x,\n", x_curr, y_curr,
                       get_pixel(pixbuf_old, x_curr, y_curr), get_pixel(pixbuf_new, x_curr, y_curr));
...
static void callback_area_updated(GdkPixbufLoader* loader, int x, int y, int width, int height, GdkPixbuf* pixbuf_old) {
...
static void callback_closed(GdkPixbufLoader* loader, GdkPixbuf* pixbuf_copy) {
    g_object_unref(pixbuf_copy);
}

/* prepare copy of pixbuf and connect other callbacks */
static void callback_area_prepared(GdkPixbufLoader* loader) {
    ...
    g_signal_connect(loader, "area-updated", (GCallback)callback_area_updated, (gpointer)pixbuf_copy);
    g_signal_connect(loader, "closed", (GCallback)callback_closed, (gpointer)pixbuf_copy);
}
...
static gsize loader_write_from_channel(GdkPixbufLoader* loader, GIOChannel* channel, gsize count_bytes) {
    guchar* buffer;
    gsize bytes_read;
...
static void test_area_updated_ico(gconstpointer data) {
    const char* filename;
    GIOChannel* channel;
    ...
    g_signal_connect(loader, "area-prepared", (GCallback)callback_area_prepared, NULL);
...
static void update_currently_loaded_frame(FrameData* frame) {
    int tmp_count;
...
static void callback_area_updated_anim(GdkPixbufLoader* loader,
                                       int x,
                                       int y,
                                       int width,
                                       int height,
...
static void callback_closed_anim(GdkPixbufLoader* loader, FrameData* frame_copy) {
...
static void callback_area_prepared_anim(GdkPixbufLoader* loader) {
    GdkPixbufAnimation* anim;
    FrameData* frame_copy = g_new(FrameData, 1);

    g_signal_connect(loader, "area-updated", (GCallback)callback_area_updated_anim, (gpointer)frame_copy);
    g_signal_connect(loader, "closed", (GCallback)callback_closed_anim, (gpointer)frame_copy);
...
static void test_area_updated_anim(gconstpointer data) {
    const char* filename;
    GIOChannel* channel;
    GdkPixbufLoader* loader;
    ...
    g_signal_connect(loader, "area-prepared", (GCallback)callback_area_prepared_anim, NULL);
