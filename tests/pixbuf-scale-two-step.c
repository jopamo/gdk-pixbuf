...
#define within_one(a, b) ((a) == (b) || (a) == (b) + 1 || (a) + 1 == (b))

static gboolean pixdata_almost_equal(GdkPixbuf* one, GdkPixbuf* two) {
    ...
    if (width_one != gdk_pixbuf_get_width(two) || height_one != gdk_pixbuf_get_height(two)) {
        g_test_fail();
    }

    for (y = 0, one_row = gdk_pixbuf_get_pixels(one), two_row = gdk_pixbuf_get_pixels(two); y < height_one;
         y++, one_row += gdk_pixbuf_get_rowstride(one), two_row += gdk_pixbuf_get_rowstride(two)) {
        for (x = 0, one_pixel = one_row, two_pixel = two_row; x < width_one;
             x++, one_pixel += gdk_pixbuf_get_n_channels(one), two_pixel += gdk_pixbuf_get_n_channels(two)) {
            if (!(within_one(one_pixel[0], two_pixel[0]) && within_one(one_pixel[1], two_pixel[1]) &&
 ...
 * optimization. */
static void test_old_and_new(gconstpointer data) {
    ...
    one = gdk_pixbuf_scale_simple(source, (int)(width * SCALE_FACTOR + 0.5), (int)(height * SCALE_FACTOR + 0.5),
                                  interp_type);
    g_unsetenv("GDK_PIXBUF_DISABLE_TWO_STEP_SCALER");
    two = gdk_pixbuf_scale_simple(source, (int)(width * SCALE_FACTOR + 0.5), (int)(height * SCALE_FACTOR + 0.5),
                                  interp_type);
...
static void crop_n_compare(const GdkPixbuf* source, /* The source image */
    ...
    gdk_pixbuf_scale(source, scaled, 0, 0, /* dest_[xy] */
                     width, height,        /* dest_width/height */
                     -1.0 * offset_x, -1.0 * offset_y, scale_factor, scale_factor, interp_type);
     ...
     * larger rowstride. */
    {
        ...
        if (scaled_width != gdk_pixbuf_get_width(cropped) || scaled_height != gdk_pixbuf_get_height(cropped)) {
            g_test_fail();
        }

        for (y = 0, scaled_row = gdk_pixbuf_get_pixels(scaled), cropped_row = gdk_pixbuf_get_pixels(cropped);
             y < scaled_height;
             y++, scaled_row += gdk_pixbuf_get_rowstride(scaled), cropped_row += gdk_pixbuf_get_rowstride(cropped)) {
            for (x = 0, scaled_pixel = scaled_row, cropped_pixel = cropped_row; x < scaled_width; x++,
                scaled_pixel += gdk_pixbuf_get_n_channels(scaled),
 ...
 * negative offsets. */
static void test_offset(gconstpointer data) {
    GdkInterpType interp_type = *(GdkInterpType*)data;
    const GdkPixbuf* source;    /* Source image */
...
static void test_dest(gconstpointer data) {
    GdkInterpType interp_type = *(GdkInterpType*)data;
    const GdkPixbuf* source;    /* Source image */
    ...
    gdk_pixbuf_scale((const GdkPixbuf*)source, copy, 0, 0, /* dest_[xy] */
                     dwidth / 2, dheight / 2,              /* dest_width/height */
                     0.0, 0.0,                             /* offset_[xy] */
                     SCALE_FACTOR, SCALE_FACTOR, interp_type);
    gdk_pixbuf_scale((const GdkPixbuf*)source, copy, dwidth / 2, 0, /* dest_[xy] */
                     dwidth / 2, dheight / 2,                       /* dest_width/height */
                     0.0, 0.0,                                      /* offset_[xy] */
                     SCALE_FACTOR, SCALE_FACTOR, interp_type);
    gdk_pixbuf_scale((const GdkPixbuf*)source, copy, 0, dheight / 2, /* dest_[xy] */
                     dwidth / 2, dheight / 2,                        /* dest_width/height */
                     0.0, 0.0,                                       /* offset_[xy] */
                     SCALE_FACTOR, SCALE_FACTOR, interp_type);
    gdk_pixbuf_scale((const GdkPixbuf*)source, copy, dwidth / 2, dheight / 2, /* dest_[xy] */
                     dwidth / 2, dheight / 2,                                 /* dest_width/height */
                     0.0, 0.0,                                                /* offset_[xy] */
                     SCALE_FACTOR, SCALE_FACTOR, interp_type);
