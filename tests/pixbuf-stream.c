...
#define compare_option(p1, p2, key) g_strcmp0(gdk_pixbuf_get_option(p1, key), gdk_pixbuf_get_option(p2, key))

static gboolean pixbuf_equal(GdkPixbuf* p1, GdkPixbuf* p2) {
    ...
    if (gdk_pixbuf_get_rowstride(p1) != gdk_pixbuf_get_rowstride(p2))
        return FALSE;
    if (memcmp(gdk_pixbuf_get_pixels(p1), gdk_pixbuf_get_pixels(p2), gdk_pixbuf_get_byte_length(p1)) != 0)
        return FALSE;
...
static void test_stream(gconstpointer data) {
...
static void async_done_cb(GObject* source, GAsyncResult* res, gpointer data) {
    GdkPixbuf* ref = data;
    GdkPixbuf* pixbuf;
    GError* error = NULL;
...
static void test_stream_async(gconstpointer data) {
    const gchar* filename = data;
    const gchar* path;
    GError* error = NULL;
    GdkPixbuf* ref;
...
static void test_stream_at_scale(gconstpointer data) {
    const gchar* filename = data;
    const gchar* path;
    GError* error = NULL;
    GdkPixbuf *pixbuf, *ref;
...
static void test_stream_at_scale_async(gconstpointer data) {
    const gchar* filename = data;
    const gchar* path;
    GError* error = NULL;
    GdkPixbuf* ref;
