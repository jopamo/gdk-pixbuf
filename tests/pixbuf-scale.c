...
static void test_scale_down(gconstpointer data) {
    const gchar* filename = data;
    const gchar* path;
    GError* error = NULL;
    GdkPixbuf* ref;
...
static void test_add_alpha(gconstpointer data) {
    const gchar* filename = data;
    const gchar* path;
    GError* error = NULL;
    GdkPixbuf* ref;
...
static void test_rotate(gconstpointer data) {
    const gchar* filename = data;
    const gchar* path;
    GError* error = NULL;
    GdkPixbuf* ref;
...
static void test_halve_checkerboard(gconstpointer data) {
    ...
    for (y = 0, row = gdk_pixbuf_get_pixels(scaled); y < scaled_height; y++, row += gdk_pixbuf_get_rowstride(scaled)) {
        for (x = 0, pixel = row; x < scaled_width; x++, pixel += gdk_pixbuf_get_n_channels(scaled)) {
            if (!(pixel[0] == expected && pixel[1] == expected && pixel[2] == expected)) {
                ...
                if (interp_type == GDK_INTERP_HYPER && (x == 0 || x == scaled_width - 1) &&
...
static void crop_n_compare(const GdkPixbuf* source,
    ...
    for (y = 0, crow = gdk_pixbuf_get_pixels(cropped), srow = gdk_pixbuf_get_pixels(scaled); y < scaled_height;
         y++, crow += gdk_pixbuf_get_rowstride(cropped), srow += gdk_pixbuf_get_rowstride(scaled)) {
        for (x = 0, cpixel = crow, spixel = srow; x < scaled_width;
             x++, cpixel += gdk_pixbuf_get_n_channels(cropped), spixel += gdk_pixbuf_get_n_channels(scaled)) {
            if (!(spixel[0] == cpixel[0] && spixel[1] == cpixel[1] && spixel[2] == cpixel[2])) {
                /* Expected failure: HYPER has a different opinion about the
                 * colors of the edge pixels */
                if (interp_type == GDK_INTERP_HYPER &&
                    ((x == 0 || x == scaled_width - 1) || (y == 0 || y == scaled_height - 1))) {
                    continue;
                }
                g_test_fail();
            }
...
static void test_dest(gconstpointer data) {
    ...
    gdk_pixbuf_scale((const GdkPixbuf*)source, copy, 0, 0, /* dest_[xy] */
                     ...
                     1.0, 1.0,                             /* scale_[xy] */
                     interp_type);
    gdk_pixbuf_scale((const GdkPixbuf*)source, copy, width / 2, 0, /* dest_[xy] */
                     width / 2, height / 2,                        /* dest_width/height */
                     0.0, 0.0,                                     /* offset_[xy] */
                     1.0, 1.0,                                     /* scale_[xy] */
                     interp_type);
    gdk_pixbuf_scale((const GdkPixbuf*)source, copy, 0, height / 2, /* dest_[xy] */
                     width / 2, height / 2,                         /* dest_width/height */
                     0.0, 0.0,                                      /* offset_[xy] */
                     1.0, 1.0,                                      /* scale_[xy] */
                     interp_type);
    gdk_pixbuf_scale((const GdkPixbuf*)source, copy, width / 2, height / 2, /* dest_[xy] */
                     width / 2, height / 2,                                 /* dest_width/height */
                     0.0, 0.0,                                              /* offset_[xy] */
                     1.0, 1.0,                                              /* scale_[xy] */
                     interp_type);
    ...
    for (y = 0, srow = gdk_pixbuf_get_pixels(source), crow = gdk_pixbuf_get_pixels(copy);
         y < gdk_pixbuf_get_height(source);
         y++, srow += gdk_pixbuf_get_rowstride(source), crow += gdk_pixbuf_get_rowstride(copy)) {
        for (x = 0, spixel = srow, cpixel = crow; x < gdk_pixbuf_get_width(source);
             x++, spixel += gdk_pixbuf_get_n_channels(source), cpixel += gdk_pixbuf_get_n_channels(copy)) {
            if (!(spixel[0] == cpixel[0] && spixel[1] == cpixel[1] && spixel[2] == cpixel[2])) {
                g_test_fail();
            }
        }
