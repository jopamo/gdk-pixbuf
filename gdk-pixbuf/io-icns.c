 ...
 */
static gboolean load_resources(unsigned size,
                               IN gpointer data,
                               gsize datalen,
                               OUT guchar** picture,
                               OUT gsize* plen,
 ...
 */
static gboolean uncompress(unsigned size, INOUT guchar** source, OUT guchar* target, INOUT gsize* _remaining) {
    guchar* data = *source;
    gsize remaining;
    gsize i = 0;
...
static GdkPixbuf* load_icon(unsigned size, IN gpointer data, gsize datalen) {
    guchar* icon = NULL;
    guchar* mask = NULL;
    ...
    if (size == 256) {
        ...
        if (!gdk_pixbuf_loader_write(loader, icon, isize, NULL) || !gdk_pixbuf_loader_close(loader, NULL)) {
...
static GdkPixbuf* icns_image_load(FILE* f, GError** error) {
    GByteArray* data;
    ...
    while (!feof(f)) {
        ...
        if (ferror(f)) {
            g_set_error(error, G_FILE_ERROR, g_file_error_from_errno(save_errno), _("Error reading ICNS image: %s"),
    ...
    if (!pixbuf)
        g_set_error_literal(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_CORRUPT_IMAGE, _("Could not decode ICNS file"));

    return pixbuf;
}

static void context_free(IcnsProgressiveState* context) {
    g_byte_array_free(context->byte_array, TRUE);
    g_clear_object(&context->pixbuf);
    g_free(context);
}

static gpointer gdk_pixbuf__icns_image_begin_load(GdkPixbufModuleSizeFunc size_func,
                                                  GdkPixbufModulePreparedFunc prepared_func,
                                                  GdkPixbufModuleUpdatedFunc updated_func,
                                                  gpointer user_data,
                                                  GError** error) {
...
static gboolean gdk_pixbuf__icns_image_stop_load(gpointer data, GError** error) {
...
static gboolean gdk_pixbuf__icns_image_load_increment(gpointer data, const guchar* buf, guint size, GError** error) {
    IcnsProgressiveState* context = data;
    int i;
    int filesize;
    gint w, h;
    ...
    filesize = (context->byte_array->data[4] << 24) | (context->byte_array->data[5] << 16) |
               (context->byte_array->data[6] << 8) | (context->byte_array->data[7]);

    if (context->byte_array->len < filesize)
        return TRUE;

    for (i = 0; i < G_N_ELEMENTS(sizes) && !context->pixbuf; i++)
        context->pixbuf = load_icon(sizes[i], context->byte_array->data, context->byte_array->len);
    ...
    (*context->updated_func)(context->pixbuf, 0, 0, gdk_pixbuf_get_width(context->pixbuf),
...
MODULE_ENTRY(fill_info)(GdkPixbufFormat* info) {
    static const GdkPixbufModulePattern signature[] = {{"icns", NULL, 100}, /* file begins with 'icns' */
                                                       {NULL, NULL, 0}};
    static const gchar* mime_types[] = {"image/x-icns", NULL};
    static const gchar* extensions[] = {"icns", NULL};
