...
static void gdk_pixbuf_set_property(GObject* object, guint prop_id, const GValue* value, GParamSpec* pspec);
static void gdk_pixbuf_get_property(GObject* object, guint prop_id, GValue* value, GParamSpec* pspec);
...
static void gdk_pixbuf_init(GdkPixbuf* pixbuf) {
...
#define PIXBUF_PARAM_FLAGS                                                                                             \
    G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_EXPLICIT_NOTIFY | G_PARAM_STATIC_NAME | G_PARAM_STATIC_NICK | \
    ...
    g_object_class_install_property(
        object_class, PROP_N_CHANNELS,
        g_param_spec_int("n-channels", _("Number of Channels"), _("The number of samples per pixel"), 0, G_MAXINT, 3,
                         PIXBUF_PARAM_FLAGS));
    /**
     ...
     */
    g_object_class_install_property(
        object_class, PROP_COLORSPACE,
        g_param_spec_enum("colorspace", _("Colorspace"), _("The colorspace in which the samples are interpreted"),
                          GDK_TYPE_COLORSPACE, GDK_COLORSPACE_RGB, PIXBUF_PARAM_FLAGS));
    /**
     * GdkPixbuf:has-alpha:
     *
     ...
     */
    g_object_class_install_property(
        object_class, PROP_HAS_ALPHA,
        g_param_spec_boolean("has-alpha", _("Has Alpha"), _("Whether the pixbuf has an alpha channel"), FALSE,
                             PIXBUF_PARAM_FLAGS));
    /**
     ...
     */
    g_object_class_install_property(object_class, PROP_BITS_PER_SAMPLE,
                                    g_param_spec_int("bits-per-sample", _("Bits per Sample"),
                                                     _("The number of bits per sample"), 1, 16, 8, PIXBUF_PARAM_FLAGS));
    /**
     * GdkPixbuf:width:
     *
    ...
    g_object_class_install_property(object_class, PROP_WIDTH,
                                    g_param_spec_int("width", _("Width"), _("The number of columns of the pixbuf"), 1,
                                                     G_MAXINT, 1, PIXBUF_PARAM_FLAGS));
    /**
     * GdkPixbuf:height:
     *
    ...
    g_object_class_install_property(object_class, PROP_HEIGHT,
                                    g_param_spec_int("height", _("Height"), _("The number of rows of the pixbuf"), 1,
                                                     G_MAXINT, 1, PIXBUF_PARAM_FLAGS));
    /**
     * GdkPixbuf:rowstride:
     *
     ...
     */
    g_object_class_install_property(
        object_class, PROP_ROWSTRIDE,
        g_param_spec_int("rowstride", _("Rowstride"),
                         _("The number of bytes between the start of a row and the start of the next row"), 1, G_MAXINT,
                         1, PIXBUF_PARAM_FLAGS));
    /**
     * GdkPixbuf:pixels:
     *
     ...
     */
    g_object_class_install_property(
        object_class, PROP_PIXELS,
        g_param_spec_pointer("pixels", _("Pixels"), _("A pointer to the pixel data of the pixbuf"),
                             PIXBUF_PARAM_FLAGS));
    /**
     ...
     * Since: 2.32
     */
    g_object_class_install_property(object_class, PROP_PIXEL_BYTES,
                                    g_param_spec_boxed("pixel-bytes", _("Pixel Bytes"), _("Readonly pixel data"),
                                                       G_TYPE_BYTES, PIXBUF_PARAM_FLAGS));
}

static void free_pixels(GdkPixbuf* pixbuf) {
...
static GBytes* gdk_pixbuf_make_bytes(GdkPixbuf* pixbuf, GError** error) {
    gchar* buffer;
    gsize size;
...
static GVariant* gdk_pixbuf_serialize(GIcon* icon) {
    GError* error = NULL;
    GVariant* result;
    GBytes* bytes;
...
static GInputStream* gdk_pixbuf_load(GLoadableIcon* icon,
                                     int size,
                                     char** type,
...
static void gdk_pixbuf_load_async(GLoadableIcon* icon,
                                  int size,
                                  GCancellable* cancellable,
...
static GInputStream* gdk_pixbuf_load_finish(GLoadableIcon* icon, GAsyncResult* res, char** type, GError** error) {
...
GdkPixbuf* gdk_pixbuf_new(GdkColorspace colorspace, gboolean has_alpha, int bits_per_sample, int width, int height) {
    ...
    rowstride = gdk_pixbuf_calculate_rowstride(colorspace, has_alpha, bits_per_sample, width, height);
    if (rowstride <= 0)
        return NULL;
    ...
    return gdk_pixbuf_new_from_data(buf, colorspace, has_alpha, bits_per_sample, width, height, rowstride, free_buffer,
                                    NULL);
}
...
GdkPixbuf* gdk_pixbuf_copy(const GdkPixbuf* pixbuf) {
    ...
    return gdk_pixbuf_new_from_data(buf, pixbuf->colorspace, pixbuf->has_alpha, pixbuf->bits_per_sample, pixbuf->width,
                                    pixbuf->height, pixbuf->rowstride, free_buffer, NULL);
}
...
GdkPixbuf* gdk_pixbuf_new_subpixbuf(GdkPixbuf* src_pixbuf, int src_x, int src_y, int width, int height) {
    ...
    pixels = (gdk_pixbuf_get_pixels(src_pixbuf) + src_y * src_pixbuf->rowstride + src_x * src_pixbuf->n_channels);

    sub = gdk_pixbuf_new_from_data(pixels, src_pixbuf->colorspace, src_pixbuf->has_alpha, src_pixbuf->bits_per_sample,
                                   width, height, src_pixbuf->rowstride, NULL, NULL);

    /* Keep a reference to src_pixbuf */
    g_object_ref(src_pixbuf);

    g_object_set_qdata_full(G_OBJECT(sub), g_quark_from_static_string("gdk-pixbuf-subpixbuf-src"), src_pixbuf,
...
static void downgrade_to_pixels(const GdkPixbuf* pixbuf) {
    switch (pixbuf->storage) {
        case STORAGE_PIXELS:
            return;
...
GBytes* gdk_pixbuf_read_pixel_bytes(const GdkPixbuf* pixbuf) {
    ...
    switch (pixbuf->storage) {
        case STORAGE_PIXELS:
            return g_bytes_new(pixbuf->s.pixels.pixels, gdk_pixbuf_get_byte_length(pixbuf));
...
const gchar* gdk_pixbuf_get_option(GdkPixbuf* pixbuf, const gchar* key) {
    ...
    options = g_object_get_qdata(G_OBJECT(pixbuf), g_quark_from_static_string("gdk_pixbuf_options"));
    if (options) {
...
gboolean gdk_pixbuf_remove_option(GdkPixbuf* pixbuf, const gchar* key) {
    ...
    if (!found) {
        ...
        g_object_set_qdata_full(G_OBJECT(pixbuf), quark, options, (GDestroyNotify)g_strfreev);
        return FALSE;
    }

    g_ptr_array_add(array, NULL);
    g_object_set_qdata_full(G_OBJECT(pixbuf), quark, g_ptr_array_free(array, FALSE), (GDestroyNotify)g_strfreev);
    g_strfreev(options);
...
gboolean gdk_pixbuf_copy_options(GdkPixbuf* src_pixbuf, GdkPixbuf* dest_pixbuf) {
    ...
    g_object_set_qdata_full(G_OBJECT(dest_pixbuf), quark, options, (GDestroyNotify)g_strfreev);
