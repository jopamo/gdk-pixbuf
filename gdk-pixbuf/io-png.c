...
static gboolean setup_png_transformations(png_structp png_read_ptr,
                                          png_infop png_info_ptr,
                                          GError** error,
                                          png_uint_32* width_p,
                                          png_uint_32* height_p,
                                          int* color_type_p) {
    ...
    */
    ...
    png_get_IHDR(png_read_ptr, png_info_ptr, &width, &height, &bit_depth, &color_type, &interlace_type,
...
static void png_simple_error_callback(png_structp png_save_ptr, png_const_charp error_msg) {
     ...
     */
    if (error && *error == NULL) {
        g_set_error(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_FAILED, _("Fatal error in PNG image file: %s"),
                    error_msg);
    }
...
static void png_simple_warning_callback(png_structp png_save_ptr, png_const_charp warning_msg) {
...
static GdkPixbuf* gdk_pixbuf__png_image_load(FILE* f, GError** error) {
    ...
    png_ptr = png_create_read_struct_2(PNG_LIBPNG_VER_STRING, error, png_simple_error_callback,
                                       png_simple_warning_callback, NULL, png_malloc_callback, png_free_callback);
#else
    png_ptr =
        png_create_read_struct(PNG_LIBPNG_VER_STRING, error, png_simple_error_callback, png_simple_warning_callback);
#endif
    if (!png_ptr)
        return NULL;
    ...
    retval = png_get_iCCP(png_ptr, info_ptr, (png_charpp)&icc_profile_title, &compression_type,
...
static gpointer gdk_pixbuf__png_image_begin_load(GdkPixbufModuleSizeFunc size_func,
...
#ifdef PNG_USER_MEM_SUPPORTED
    lc->png_read_ptr = png_create_read_struct_2(PNG_LIBPNG_VER_STRING, lc, /* error/warning callback data */
                                                png_error_callback, png_warning_callback, NULL, png_malloc_callback,
                                                png_free_callback);
#else
    lc->png_read_ptr = png_create_read_struct(PNG_LIBPNG_VER_STRING, lc, /* error/warning callback data */
    ...
    png_set_progressive_read_fn(lc->png_read_ptr, lc, /* callback data */
...
static gboolean gdk_pixbuf__png_image_stop_load(gpointer context, GError** error) {
...
static gboolean gdk_pixbuf__png_image_load_increment(gpointer context, const guchar* buf, guint size, GError** error) {
    LoadContext* lc = context;
    ...
    if (setjmp(png_jmpbuf(lc->png_read_ptr))) {
        ...
        png_process_data(lc->png_read_ptr, lc->png_info_ptr, (guchar*)buf, size);
    }
    ...
    if (lc->fatal_error_occurred) {
        ...
        if (lc->first_row_seen_in_chunk >= 0) {
            ...
            if (pass_diff == 0) {
                ...
                (lc->updated_func)(lc->pixbuf, 0, lc->first_row_seen_in_chunk, width,
                                   (lc->last_row_seen_in_chunk - lc->first_row_seen_in_chunk) + 1,
                                   lc->notify_user_data);
            }
            else if (pass_diff == 1) {
                ...
                /* first row to end */
                (lc->updated_func)(lc->pixbuf, 0, lc->first_row_seen_in_chunk, width,
                                   (lc->max_row_seen_in_chunk - lc->first_row_seen_in_chunk) + 1, lc->notify_user_data);
                /* top to last row */
                (lc->updated_func)(lc->pixbuf, 0, 0, width, lc->last_row_seen_in_chunk + 1, lc->notify_user_data);
            }
            else {
                /* We made at least one entire pass, so update the
                   whole image */
                (lc->updated_func)(lc->pixbuf, 0, 0, width, lc->max_row_seen_in_chunk + 1, lc->notify_user_data);
            }
        }
...
static void png_info_callback(png_structp png_read_ptr, png_infop png_info_ptr) {
    ...
    if (!setup_png_transformations(lc->png_read_ptr, lc->png_info_ptr, lc->error, &width, &height, &color_type)) {
...

    {
        ...
        if (w == 0 || h == 0) {
            lc->fatal_error_occurred = TRUE;
            g_set_error_literal(lc->error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_FAILED,
                                _("Transformed PNG has zero width or height."));
            return;
        }
    }
    ...
    if (png_get_text(png_read_ptr, png_info_ptr, &png_text_ptr, &num_texts)) {
        for (i = 0; i < num_texts; i++) {
            ...
            if (png_text_to_pixbuf_option(png_text_ptr[i], &key, &value)) {
...
#if defined(PNG_cHRM_SUPPORTED)
    /* Extract embedded ICC profile */
    retval = png_get_iCCP(png_read_ptr, png_info_ptr, (png_charpp)&icc_profile_title, &compression_type,
...
static void png_error_callback(png_structp png_read_ptr, png_const_charp error_msg) {
    ...
    if (lc->error && *lc->error == NULL) {
        g_set_error(lc->error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_CORRUPT_IMAGE,
                    _("Fatal error reading PNG image file: %s"), error_msg);
    }
...
static void png_save_to_callback_write_func(png_structp png_ptr, png_bytep data, png_size_t length) {
...
static gboolean real_save_png(GdkPixbuf* pixbuf,
    ...
    for (int i = 0; i < n_keys; i++) {
        ...
        if (strncmp(key, "tEXt::", 6) == 0) {
            ...
            if (text.text == NULL) {
                g_set_error(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_BAD_OPTION,
                            _("Value for PNG text chunk '%s' cannot be converted to ISO-8859-1 encoding."),
                            unprefixed_key);
                success = FALSE;
                goto cleanup;
            }
            ...
            if (icc_profile_size < 127) {
                g_set_error(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_BAD_OPTION,
                            _("Color profile has invalid length %d"), (int)icc_profile_size);
                success = FALSE;
                goto cleanup;
            }
        }
            ...
            if (endptr == value || (compression < 0 || compression > 9)) {
                g_set_error(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_BAD_OPTION,
                            _("PNG compression level must be a value between 0 and 9; value “%s” is invalid"), value);
                success = FALSE;
                goto cleanup;
            }
        }
            ...
            if (endptr == value || dpi <= 0) {
                g_set_error(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_BAD_OPTION,
                            ...
                            is_horizontal ? "x-dpi" : "y-dpi", value);
                success = FALSE;
                goto cleanup;
            }
    ...
    png_ptr =
        png_create_write_struct(PNG_LIBPNG_VER_STRING, error, png_simple_error_callback, png_simple_warning_callback);
    if (png_ptr == NULL) {
        success = FALSE;
        goto cleanup;
    }
    ...
    if (to_callback) {
        ...
        png_set_write_fn(png_ptr, &to_callback_ioptr, png_save_to_callback_write_func, png_save_to_callback_flush_func);
    }
    else {
        png_init_io(png_ptr, f);
    }
    ...
    if (icc_profile != NULL) {
        png_set_iCCP(png_ptr, info_ptr, "ICC profile", PNG_COMPRESSION_TYPE_BASE, (png_bytep)icc_profile,
                     icc_profile_size);
    }
#endif

    if (has_alpha) {
        png_set_IHDR(png_ptr, info_ptr, w, h, bpc, PNG_COLOR_TYPE_RGB_ALPHA, PNG_INTERLACE_NONE,
                     PNG_COMPRESSION_TYPE_BASE, PNG_FILTER_TYPE_BASE);
    }
    else {
        png_set_IHDR(png_ptr, info_ptr, w, h, bpc, PNG_COLOR_TYPE_RGB, PNG_INTERLACE_NONE, PNG_COMPRESSION_TYPE_BASE,
...
static gboolean gdk_pixbuf__png_image_save(FILE* f, GdkPixbuf* pixbuf, gchar** keys, gchar** values, GError** error) {
    int n_keys = keys != NULL ? g_strv_length(keys) : 0;

    return real_save_png(pixbuf, n_keys, keys, values, error, FALSE, f, NULL, NULL);
}

static gboolean gdk_pixbuf__png_image_save_to_callback(GdkPixbufSaveFunc save_func,
                                                       gpointer user_data,
                                                       GdkPixbuf* pixbuf,
                                                       gchar** keys,
                                                       gchar** values,
    ...
    return real_save_png(pixbuf, n_keys, keys, values, error, TRUE, NULL, save_func, user_data);
}

static gboolean gdk_pixbuf__png_is_save_option_supported(const gchar* option_key) {
    if (g_strcmp0(option_key, "compression") == 0 || g_strcmp0(option_key, "icc-profile") == 0 ||
        g_strcmp0(option_key, "x-dpi") == 0 || g_strcmp0(option_key, "y-dpi") == 0 ||
