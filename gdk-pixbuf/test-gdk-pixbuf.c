...
static void store_pixel(guchar* pixels, int pixel, gboolean alpha) {
    if (alpha) {
        pixels[0] = pixel >> 24;
        pixels[1] = pixel >> 16;
        pixels[2] = pixel >> 8;
...
static void fill_with_pixel(GdkPixbuf* pixbuf, int pixel) {
    int x, y;

    for (x = 0; x < gdk_pixbuf_get_width(pixbuf); x++) {
        for (y = 0; y < gdk_pixbuf_get_height(pixbuf); y++) {
            store_pixel(gdk_pixbuf_get_pixels(pixbuf) + y * gdk_pixbuf_get_rowstride(pixbuf) +
                            x * gdk_pixbuf_get_n_channels(pixbuf),
                        pixel, gdk_pixbuf_get_has_alpha(pixbuf));
        }
    }
}

static int load_pixel(const guchar* pixels, gboolean alpha) {
    if (alpha)
...
static gboolean simple_composite_test_one(GdkInterpType type,
                                          int source_pixel,
    ...
    gdk_pixbuf_composite(source_pixbuf, destination_pixbuf, 0, 0, 32, 32, 0, 0, 1, 1, type, 0xFF);

    result_pixel =
        load_pixel(gdk_pixbuf_get_pixels(destination_pixbuf) + 16 * gdk_pixbuf_get_rowstride(destination_pixbuf) +
    ...
    if (result_pixel != expected_result) {
        ...
        switch (type) {
            case GDK_INTERP_NEAREST:
                interpolation_type = "GDK_INTERP_NEAREST";
                break;
            case GDK_INTERP_TILES:
                interpolation_type = "GDK_INTERP_TILES";
                break;
            case GDK_INTERP_BILINEAR:
                interpolation_type = "GDK_INTERP_BILINEAR";
                break;
            case GDK_INTERP_HYPER:
                interpolation_type = "GDK_INTERP_HYPER";
                break;
            default:
                interpolation_type = "???";
        }
        ...
        g_message("simple_composite_test (%s): composite %s on top of %s, expected %s, got %s", interpolation_type,
...
static gboolean simple_composite_test_one_type(GdkInterpType type) {
    gboolean success;
...
static gboolean simple_composite_test(void) {
    gboolean success;
