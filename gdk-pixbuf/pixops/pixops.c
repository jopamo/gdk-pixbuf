...
static void _pixops_scale_real(guchar* dest_buf,
                               int render_x0,
                               int render_y0,
                               int render_x1,
                               int render_y1,
...
typedef guchar* (*PixopsLineFunc)(int* weights,
                                  int n_x,
                                  int n_y,
                                  guchar* dest,
                                  int dest_x,
                                  ...
                                  guint32 color1,
                                  guint32 color2);
typedef void (*PixopsPixelFunc)(guchar* dest,
                                int dest_x,
                                int dest_channels,
                                int dest_has_alpha,
                                int src_has_alpha,
 ...
 *
 */
static void _pixops_use_medialib() {
...
static void pixops_scale_nearest(guchar* dest_buf,
                                 int render_x0,
                                 int render_y0,
                                 int render_x1,
                                 int render_y1,
...
static void pixops_composite_nearest(guchar* dest_buf,
    ...
    for (i = 0; i < (render_y1 - render_y0); i++) {
        ...
        INNER_LOOP(
            src_channels, dest_channels, if (src_has_alpha) a0 = (p[3] * overall_alpha) / 0xff; else a0 = overall_alpha;
...
static void pixops_composite_nearest_noscale(guchar* dest_buf,
                                             int render_x0,
                                             int render_y0,
                                             int render_x1,
                                             int render_y1,
...
static void pixops_composite_color_nearest(guchar* dest_buf,
                                           int render_x0,
                                           int render_y0,
                                           int render_x1,
                                           int render_y1,
    ...
    for (i = 0; i < (render_y1 - render_y0); i++) {
        ...
        j = 0;
        INNER_LOOP(
            src_channels, dest_channels, if (src_has_alpha) a0 = (p[3] * overall_alpha + 0xff) >> 8;
            ...
            if (dest_channels == 4) dest[3] = 0xff;
...
static void composite_pixel(guchar* dest,
                            int dest_x,
                            int dest_channels,
                            int dest_has_alpha,
                            int src_has_alpha,
...
static guchar* composite_line(int* weights,
                              int n_x,
                              int n_y,
                              guchar* dest,
                              int dest_x,
...
static guchar* composite_line_22_4a4(int* weights,
                                     int n_x,
                                     int n_y,
                                     guchar* dest,
                                     int dest_x,
...
static void composite_pixel_color(guchar* dest,
                                  int dest_x,
                                  int dest_channels,
                                  int dest_has_alpha,
                                  int src_has_alpha,
...
static guchar* composite_line_color(int* weights,
                                    int n_x,
                                    int n_y,
                                    guchar* dest,
                                    int dest_x,
...
static void scale_pixel(guchar* dest,
                        int dest_x,
                        int dest_channels,
                        int dest_has_alpha,
                        int src_has_alpha,
...
static guchar* scale_line(int* weights,
                          int n_x,
                          int n_y,
                          guchar* dest,
                          int dest_x,
    ...
    while (dest < dest_end) {
        ...
        pixel_weights = weights + ((x >> (SCALE_SHIFT - SUBSAMPLE_BITS)) & SUBSAMPLE_MASK) * n_xy;
...
static guchar* scale_line_22_33(int* weights,
                                int n_x,
                                int n_y,
                                guchar* dest,
                                int dest_x,
...
static void process_pixel(int* weights,
                          int n_x,
                          int n_y,
                          guchar* dest,
                          int dest_x,
...
static void correct_total(int* weights, int n_x, int n_y, int total, double overall_alpha) {
        ...
        for (d = 1, c = correction; c != 0 && remaining != 0; d++, c = correction / d)
            for (i = n_x * n_y - 1; i >= 0 && c != 0 && remaining != 0; i--)
                if (*(weights + i) + c >= 0) {
                    *(weights + i) += c;
                    remaining -= c;
                    if ((0 < remaining && remaining < c) || (0 > remaining && remaining > c))
                        c = remaining;
                }
    }
}
...
static int* make_filter_table(PixopsFilter* filter) {
    ...
    for (i_offset = 0; i_offset < SUBSAMPLE; i_offset++)
        for (j_offset = 0; j_offset < SUBSAMPLE; j_offset++) {
            ...
            for (i = 0; i < n_y; i++)
                for (j = 0; j < n_x; j++) {
                    weight = filter->x.weights[(j_offset * n_x) + j] * filter->y.weights[(i_offset * n_y) + i] *
                                 filter->overall_alpha * 65536 +
                             0.5;
...
#define MYDIV(a, b) ((a) > 0 ? (a) / (b) : ((a) - (b) + 1) / (b)) /* Division so that -1/5 = -1 */
    ...
    for (i = 0; i < (render_y1 - render_y0); i++) {
        ...
        int x_start;
        int* run_weights = filter_weights + ((y >> (SCALE_SHIFT - SUBSAMPLE_BITS)) & SUBSAMPLE_MASK) * filter->x.n *
                                                filter->y.n * SUBSAMPLE;
        guchar* new_outbuf;
        ...
        while (x_start < 0 && outbuf < outbuf_end) {
            process_pixel(
                run_weights + ((x >> (SCALE_SHIFT - SUBSAMPLE_BITS)) & SUBSAMPLE_MASK) * (filter->x.n * filter->y.n),
                filter->x.n, filter->y.n, outbuf, dest_x, dest_channels, dest_has_alpha, line_bufs, src_channels,
                src_has_alpha, x >> SCALE_SHIFT, src_width, check_size, tcolor1, tcolor2, pixel_func);
        ...
        new_outbuf = (*line_func)(run_weights, filter->x.n, filter->y.n, outbuf, dest_x,
                                  dest_buf + (gsize)dest_rowstride * i + run_end_index * dest_channels, dest_channels,
                                  dest_has_alpha, line_bufs, src_channels, src_has_alpha, x, x_step, src_width,
 ...
 */
static gboolean tile_make_weights(PixopsFilterDimension* dim, double scale) {
 ...
 */
static gboolean bilinear_magnify_make_weights(PixopsFilterDimension* dim, double scale) {
    double* pixel_weights;
    int n;
    int offset;
    int i;
 ...
 */
static gboolean bilinear_box_make_weights(PixopsFilterDimension* dim, double scale) {
...
static gboolean make_weights(PixopsFilter* filter, PixopsInterpType interp_type, double scale_x, double scale_y) {
    switch (interp_type) {
...
static gboolean need_to_prescale(double scale_x, double scale_y, PixopsInterpType interp_type) {
 ...
 */
static guchar* prescale(const guchar** src_bufp,
                        int* src_widthp,
                        int* src_heightp,
                        int* src_rowstridep,
                        int src_channels,
    ...
    _pixops_scale(tmp_buf, tmp_width, tmp_height, tmp_rowstride, tmp_channels, tmp_has_alpha, src_buf, src_width,
                  src_height, src_rowstride, src_channels, src_has_alpha, 0, 0, tmp_width, tmp_height, 0.0, 0.0,
                  prescale_x, prescale_y, interp_type);
...
static void _pixops_composite_color_real(guchar* dest_buf,
                                         int render_x0,
                                         int render_y0,
                                         int render_x1,
                                         int render_y1,
    ...
    if (interp_type == PIXOPS_INTERP_NEAREST) {
        pixops_composite_color_nearest(dest_buf, render_x0, render_y0, render_x1, render_y1, dest_rowstride,
                                       dest_channels, dest_has_alpha, src_buf, src_width, src_height, src_rowstride,
                                       src_channels, src_has_alpha, scale_x, scale_y, overall_alpha, check_x, check_y,
                                       check_size, color1, color2);
        return;
    }

    if (need_to_prescale(scale_x, scale_y, interp_type))
        tmp_buf = prescale(&src_buf, &src_width, &src_height, &src_rowstride, src_channels, src_has_alpha, &scale_x,
                           &scale_y, interp_type);
    ...
    pixops_process(dest_buf, render_x0, render_y0, render_x1, render_y1, dest_rowstride, dest_channels, dest_has_alpha,
                   src_buf, src_width, src_height, src_rowstride, src_channels, src_has_alpha, scale_x, scale_y,
                   check_x, check_y, check_size, color1, color2, &filter, line_func, composite_pixel_color);
...
void _pixops_composite_color(guchar* dest_buf,
    ...
    if (!src_has_alpha && overall_alpha == 255) {
        _pixops_scale(dest_buf, dest_width, dest_height, dest_rowstride, dest_channels, dest_has_alpha, src_buf,
                      src_width, src_height, src_rowstride, src_channels, src_has_alpha, dest_x, dest_y,
                      dest_region_width, dest_region_height, offset_x, offset_y, scale_x, scale_y, interp_type);
        return;
    }
    ...
    _pixops_composite_color_real(new_dest_buf, render_x0, render_y0, render_x1, render_y1, dest_rowstride,
                                 dest_channels, dest_has_alpha, src_buf, src_width, src_height, src_rowstride,
                                 src_channels, src_has_alpha, scale_x, scale_y, (PixopsInterpType)interp_type,
 ...
 **/
static void _pixops_composite_real(guchar* dest_buf,
                                   int render_x0,
                                   int render_y0,
                                   int render_x1,
                                   int render_y1,
    ...
    if (interp_type == PIXOPS_INTERP_NEAREST) {
        if (scale_x == 1.0 && scale_y == 1.0)
            pixops_composite_nearest_noscale(dest_buf, render_x0, render_y0, render_x1, render_y1, dest_rowstride,
                                             dest_channels, dest_has_alpha, src_buf, src_width, src_height,
                                             src_rowstride, src_channels, src_has_alpha, overall_alpha);
        else
            pixops_composite_nearest(dest_buf, render_x0, render_y0, render_x1, render_y1, dest_rowstride,
                                     dest_channels, dest_has_alpha, src_buf, src_width, src_height, src_rowstride,
                                     src_channels, src_has_alpha, scale_x, scale_y, overall_alpha);
        return;
    }
    ...
    if (filter.x.n == 2 && filter.y.n == 2 && dest_channels == 4 && src_channels == 4 && src_has_alpha &&
        !dest_has_alpha)
    ...
    pixops_process(dest_buf, render_x0, render_y0, render_x1, render_y1, dest_rowstride, dest_channels, dest_has_alpha,
                   src_buf, src_width, src_height, src_rowstride, src_channels, src_has_alpha, scale_x, scale_y, 0, 0,
                   0, 0, 0, &filter, line_func, composite_pixel);
...
void _pixops_composite(guchar* dest_buf,
...
#ifdef USE_MEDIALIB
    pixops_medialib_composite(dest_buf, dest_width, dest_height, dest_rowstride, dest_channels, dest_has_alpha, src_buf,
                              src_width, src_height, src_rowstride, src_channels, src_has_alpha, dest_x, dest_y,
                              dest_region_width, dest_region_height, offset_x, offset_y, scale_x, scale_y,
    ...
    _pixops_composite_real(new_dest_buf, render_x0, render_y0, render_x1, render_y1, dest_rowstride, dest_channels,
                           dest_has_alpha, src_buf, src_width, src_height, src_rowstride, src_channels, src_has_alpha,
                           scale_x, scale_y, (PixopsInterpType)interp_type, overall_alpha);
}

#ifdef USE_MEDIALIB
static void medialib_get_interpolation(mlInterp* ml_interp,
                                       PixopsInterpType interp_type,
                                       double scale_x,
                                       double scale_y,
                                       double overall_alpha) {
    ...
    ml_interp->interp_table = (void*)mlib_ImageInterpTableCreate(
        MLIB_DOUBLE, ml_interp->po_filter.x.n, ml_interp->po_filter.y.n, leftPadding, topPadding, SUBSAMPLE_BITS,
        SUBSAMPLE_BITS, 8, ml_interp->po_filter.x.weights, ml_interp->po_filter.y.weights);
...
static void pixops_medialib_composite(guchar* dest_buf,
    ...
    if (!use_medialib) {
        ...
        _pixops_composite_real(dest_buf + (gsize)dest_y * dest_rowstride + (gsize)dest_x * dest_channels,
                               dest_x - offset_x, dest_y - offset_y, dest_x + dest_region_width - offset_x,
                               dest_y + dest_region_height - offset_y, dest_rowstride, dest_channels, dest_has_alpha,
                               src_buf, src_width, src_height, src_rowstride, src_channels, src_has_alpha, scale_x,
        ...
        if (!src_has_alpha && overall_alpha == 255 && dest_channels <= src_channels) {
            pixops_medialib_scale(dest_buf, dest_region_width, dest_region_height, dest_rowstride, dest_channels,
                                  dest_has_alpha, src_buf, src_width, src_height, src_rowstride, src_channels,
                                  src_has_alpha, dest_x, dest_y, dest_region_width, dest_region_height, offset_x,
                                  offset_y, scale_x, scale_y, interp_type);
            return;
        }

        mlib_ImageSetStruct(&img_src, MLIB_BYTE, src_channels, src_width, src_height, src_rowstride, src_buf);

        if (dest_x == 0 && dest_y == 0 && dest_width == dest_region_width && dest_height == dest_region_height) {
            mlib_ImageSetStruct(&img_dest, MLIB_BYTE, dest_channels, dest_width, dest_height, dest_rowstride, dest_buf);
        }
        else {
            mlib_u8* data = dest_buf + (gsize)dest_y * dest_rowstride + (gsize)dest_x * dest_channels;

            mlib_ImageSetStruct(&img_dest, MLIB_BYTE, dest_channels, dest_region_width, dest_region_height,
                                dest_rowstride, data);
        }
        ...
        if (interp_type == PIXOPS_INTERP_NEAREST) {
            ...
            mlib_ImageZoomTranslateBlend(&img_dest, &img_src, scale_x, scale_y, ml_offset_x, ml_offset_y, MLIB_NEAREST,
                                         MLIB_EDGE_SRC_EXTEND_INDEF, blend, overall_alpha, 1);
        }
        else {
            blend = src_has_alpha ? MLIB_BLEND_GTK_SRC_OVER : MLIB_BLEND_GTK_SRC;

            if (interp_type == PIXOPS_INTERP_BILINEAR && scale_x > 1.0 && scale_y > 1.0) {
                mlib_ImageZoomTranslateBlend(&img_dest, &img_src, scale_x, scale_y, ml_offset_x, ml_offset_y,
                                             MLIB_BILINEAR, MLIB_EDGE_SRC_EXTEND_INDEF, blend, overall_alpha, 1);
            }
            else {
                medialib_get_interpolation(&ml_interp, interp_type, scale_x, scale_y, overall_alpha / 255.0);

                if (ml_interp.interp_table != NULL) {
                    mlib_ImageZoomTranslateTableBlend(&img_dest, &img_src, scale_x, scale_y, ml_offset_x + ml_interp.tx,
                                                      ml_offset_y + ml_interp.ty, ml_interp.interp_table,
                                                      MLIB_EDGE_SRC_EXTEND_INDEF, blend, 1);
                    ...
                    _pixops_composite_real(dest_buf + (gsize)dest_y * dest_rowstride + (gsize)dest_x * dest_channels,
                                           dest_x - offset_x, dest_y - offset_y, dest_x + dest_region_width - offset_x,
                                           dest_y + dest_region_height - offset_y, dest_rowstride, dest_channels,
                                           dest_has_alpha, src_buf, src_width, src_height, src_rowstride, src_channels,
                                           src_has_alpha, scale_x, scale_y, interp_type, overall_alpha);
                }
            }
        }
    }
...
static void _pixops_scale_real(guchar* dest_buf,
    ...
    if (interp_type == PIXOPS_INTERP_NEAREST) {
        pixops_scale_nearest(dest_buf, render_x0, render_y0, render_x1, render_y1, dest_rowstride, dest_channels,
                             dest_has_alpha, src_buf, src_width, src_height, src_rowstride, src_channels, src_has_alpha,
                             scale_x, scale_y);
        return;
    }
    ...
    pixops_process(dest_buf, render_x0, render_y0, render_x1, render_y1, dest_rowstride, dest_channels, dest_has_alpha,
                   src_buf, src_width, src_height, src_rowstride, src_channels, src_has_alpha, scale_x, scale_y, 0, 0,
                   0, 0, 0, &filter, line_func, scale_pixel);
...
void _pixops_scale(guchar* dest_buf,
...
#ifdef USE_MEDIALIB
    pixops_medialib_scale(dest_buf, dest_width, dest_height, dest_rowstride, dest_channels, dest_has_alpha, src_buf,
                          src_width, src_height, src_rowstride, src_channels, src_has_alpha, dest_x, dest_y,
                          dest_region_width, dest_region_height, offset_x, offset_y, scale_x, scale_y,
                          (PixopsInterpType)interp_type);
    return;
    ...
    _pixops_scale_real(new_dest_buf, render_x0, render_y0, render_x1, render_y1, dest_rowstride, dest_channels,
                       dest_has_alpha, src_buf, src_width, src_height, src_rowstride, src_channels, src_has_alpha,
...
static void pixops_medialib_scale(guchar* dest_buf,
     ...
     */
    if (!use_medialib) {
        _pixops_scale_real(dest_buf + (gsize)dest_y * dest_rowstride + (gsize)dest_x * dest_channels, dest_x - offset_x,
                           dest_y - offset_y, dest_x + dest_region_width - offset_x,
                           dest_y + dest_region_height - offset_y, dest_rowstride, dest_channels, dest_has_alpha,
                           src_buf, src_width, src_height, src_rowstride, src_channels, src_has_alpha, scale_x, scale_y,
                           interp_type);
    }
        ...
        mlib_ImageSetStruct(&img_orig_src, MLIB_BYTE, src_channels, src_width, src_height, src_rowstride, src_buf);
        ...
        if (!src_has_alpha) {
            if (src_channels > dest_channels) {
                ...
                if (src_buf != NULL) {
                    ...
                    mlib_ImageSetStruct(&img_src, MLIB_BYTE, src_channels, src_width, src_height, src_rowstride,
                                        tmp_buf);
        ...
        if (interp_type == PIXOPS_INTERP_NEAREST) {
            if (src_channels == dest_channels) {
                mlib_ImageZoomTranslate(&img_dest, &img_src, scale_x, scale_y, ml_offset_x, ml_offset_y, MLIB_NEAREST,
                                        MLIB_EDGE_SRC_EXTEND_INDEF);
            }
            else {
                mlib_ImageZoomTranslateBlend(&img_dest, &img_src, scale_x, scale_y, ml_offset_x, ml_offset_y,
                                             MLIB_NEAREST, MLIB_EDGE_SRC_EXTEND_INDEF, MLIB_BLEND_GTK_SRC, 1.0, 1);
            }
        }
        else if (src_channels == dest_channels && !src_has_alpha) {
            if (interp_type == PIXOPS_INTERP_BILINEAR && scale_x > 1.0 && scale_y > 1.0) {
                mlib_ImageZoomTranslate(&img_dest, &img_src, scale_x, scale_y, ml_offset_x, ml_offset_y, MLIB_BILINEAR,
                                        MLIB_EDGE_SRC_EXTEND_INDEF);
            }
            else {
                medialib_get_interpolation(&ml_interp, interp_type, scale_x, scale_y, 1.0);

                if (ml_interp.interp_table != NULL) {
                    mlib_ImageZoomTranslateTable(&img_dest, &img_src, scale_x, scale_y, ml_offset_x + ml_interp.tx,
                                                 ml_offset_y + ml_interp.ty, ml_interp.interp_table,
                                                 MLIB_EDGE_SRC_EXTEND_INDEF);
                    ...
                    mlib_ImageZoomTranslate(&img_dest, &img_src, scale_x, scale_y, ml_offset_x, ml_offset_y, ml_filter,
                                            MLIB_EDGE_SRC_EXTEND_INDEF);
                }
            }
        }
        ...
        /* Deal with case where src_channels != dest_channels || src_has_alpha */
        else if (interp_type == PIXOPS_INTERP_BILINEAR && scale_x > 1.0 && scale_y > 1.0) {
            mlib_ImageZoomTranslateBlend(&img_dest, &img_src, scale_x, scale_y, ml_offset_x, ml_offset_y, MLIB_BILINEAR,
                                         MLIB_EDGE_SRC_EXTEND_INDEF, MLIB_BLEND_GTK_SRC, 1.0, 1);
        }
        else {
            medialib_get_interpolation(&ml_interp, interp_type, scale_x, scale_y, 1.0);
            ...
            if (ml_interp.interp_table != NULL) {
                mlib_ImageZoomTranslateTableBlend(&img_dest, &img_src, scale_x, scale_y, ml_offset_x + ml_interp.tx,
                                                  ml_offset_y + ml_interp.ty, ml_interp.interp_table,
                                                  MLIB_EDGE_SRC_EXTEND_INDEF, MLIB_BLEND_GTK_SRC, 1);
                mlib_ImageInterpTableDelete(ml_interp.interp_table);
            }
            else {
                mlib_filter ml_filter;
