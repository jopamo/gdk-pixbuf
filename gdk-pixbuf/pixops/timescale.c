...
static double stop_timing(const char* test, int iterations, int bytes) {
    guint64 stop_time;
    double msecs;
    ...
    printf("%s%d\t%.1f\t\t%.2f\t\t%.2f\n", test, iterations, msecs, msecs / iterations,
           ((double)bytes * iterations) / (1000 * msecs));

    return ((double)bytes * iterations) / (1000 * msecs);
}

static void init_array(double times[3][3][4]) {
    int i, j, k;
...
static void dump_array(double times[3][3][4]) {
    int i, j;
    ...
    for (i = 0; i < 3; i++) {
        for (j = 0; j < 4; j++) {
            ...
            printf("%6.2f  %6.2f   %6.2f", times[i][0][j], times[i][1][j], times[i][2][j]);
    ...
    for (src_index = 0; src_index < 3; src_index++)
        for (dest_index = 0; dest_index < 3; dest_index++) {
            ...
            for (filter_level = PIXOPS_INTERP_NEAREST; filter_level <= PIXOPS_INTERP_HYPER; filter_level++) {
                printf("src_channels = %d (%s); dest_channels = %d (%s); filter_level=", src_channels,
                       src_has_alpha ? "alpha" : "no alpha", dest_channels, dest_has_alpha ? "alpha" : "no alpha");
                switch (filter_level) {
                    case PIXOPS_INTERP_NEAREST:
                ...
                if (!(src_has_alpha && !dest_has_alpha)) {
                    start_timing();
                    for (i = 0; i < ITERS; i++) {
                        _pixops_scale(dest_buf, dest_width, dest_height, dest_rowstride, dest_channels, dest_has_alpha,
                                      src_buf, src_width, src_height, src_rowstride, src_channels, src_has_alpha, 0, 0,
                                      0, 0, 0, 0, (double)dest_width / src_width, (double)dest_height / src_height,
                                      filter_level);
                    }
                ...
                start_timing();
                for (i = 0; i < ITERS; i++) {
                    _pixops_composite(dest_buf, dest_width, dest_height, dest_rowstride, dest_channels, dest_has_alpha,
                                      src_buf, src_width, src_height, src_rowstride, src_channels, src_has_alpha, 0, 0,
                                      0, 0, 0, 0, (double)dest_width / src_width, (double)dest_height / src_height,
                                      filter_level, 255);
                }
                ...
                start_timing();
                for (i = 0; i < ITERS; i++) {
                    _pixops_composite_color(dest_buf, dest_width, dest_height, dest_rowstride, dest_channels,
                                            dest_has_alpha, src_buf, src_width, src_height, src_rowstride, src_channels,
                                            src_has_alpha, 0, 0, 0, 0, 0, 0, (double)dest_width / src_width,
                                            (double)dest_height / src_height, filter_level, 255, 0, 0, 16, 0xaaaaaa,
                                            0x555555);
                }
