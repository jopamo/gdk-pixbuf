...
#define return_header_corrupt(error)                                                                             \
    {                                                                                                            \
        g_set_error_literal(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_CORRUPT_IMAGE, _("Image header corrupt")); \
        return FALSE;                                                                                            \
    }
#define return_invalid_format(error)                                                                            \
    {                                                                                                           \
        g_set_error_literal(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_UNKNOWN_TYPE, _("Image format unknown")); \
        return FALSE;                                                                                           \
    }
...
gboolean gdk_pixdata_deserialize(GdkPixdata* pixdata, guint stream_length, const guint8* stream, GError** error) {
    ...
    if ((color_type != GDK_PIXDATA_COLOR_TYPE_RGB && color_type != GDK_PIXDATA_COLOR_TYPE_RGBA) ||
...
static gboolean diff2_rgb(const guint8* ip) {
    return ip[0] != ip[3] || ip[1] != ip[4] || ip[2] != ip[5];
}

static gboolean diff2_rgba(const guint8* ip) {
...
gpointer gdk_pixdata_from_pixbuf(GdkPixdata* pixdata, const GdkPixbuf* pixbuf, gboolean use_rle) {
    ...
    g_return_val_if_fail(
        (pixbuf->n_channels == 3 && !pixbuf->has_alpha) || (pixbuf->n_channels == 4 && pixbuf->has_alpha), NULL);
    ...
    if (encoding == GDK_PIXDATA_ENCODING_RLE) {
        ...
        if (n_bytes % bpp != 0) {
            ...
            buf = gdk_pixbuf_new_from_data(data, GDK_COLORSPACE_RGB, pixbuf->has_alpha, 8, pixbuf->width,
                                           pixbuf->height, rowstride, free_buffer, NULL);
            gdk_pixbuf_copy_area(pixbuf, 0, 0, pixbuf->width, pixbuf->height, buf, 0, 0);
        }
        else
        ...
        free_me = data;
        img_buffer = data;
        img_buffer_end = rl_encode_rgbx(img_buffer, pixels, pixels + n_bytes, bpp);
    ...
    if (copy_pixels) {
        ...
        if (!data) {
            g_set_error(
                ...
                g_dngettext(GETTEXT_PACKAGE, "failed to allocate image buffer of %u byte",
                            "failed to allocate image buffer of %u bytes", pixdata->rowstride * pixdata->height),
...
static inline void save_uchar(CSourceData* cdata, guint8 d) {
...
static inline void save_rle_decoder(GString* gstring,
...
GString* gdk_pixdata_to_csource(GdkPixdata* pixdata, const gchar* name, GdkPixdataDumpType dump_type) {
    ...
    /* initial comment
     */
    APPEND(gstring, "/* GdkPixbuf %s C-Source image dump %s*/\n\n", bpp > 3 ? "RGBA" : "RGB",
           rle_encoded ? "1-byte-run-length-encoded " : "");

    /* dump RLE decoder for structures
     */
    if (cdata.dump_rle_decoder && cdata.dump_struct)
        save_rle_decoder(gstring, macro_name, cdata.dump_gtypes ? "guint" : "unsigned int",
