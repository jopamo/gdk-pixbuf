...
void output_int(FILE* outfile, guint32 num, const char* comment) {
    guchar bytes[4];
    ...
    g_fprintf(outfile, "  /* %s (%u) */\n  0x%.2x, 0x%.2x, 0x%.2x, 0x%.2x,\n", comment, num, bytes[0], bytes[1],
              bytes[2], bytes[3]);
}

void output_bool(FILE* outfile, gboolean val, const char* comment) {
    g_fprintf(outfile, "  /* %s (%s) */\n  0x%.2x,\n", comment, val ? "TRUE" : "FALSE", val ? 1 : 0);
}

void output_pixbuf(FILE* outfile, gboolean ext_symbols, const char* varname, GdkPixbuf* pixbuf) {
    const char* modifier;
    const guchar* p;
    const guchar* end;
    gboolean has_alpha;
...
void usage(void) {
    g_fprintf(stderr,
              "Usage: make-inline-pixbuf [--extern-symbols] OUTFILE varname1 imagefile1 varname2 imagefile2 ...\n");
    exit(1);
}
