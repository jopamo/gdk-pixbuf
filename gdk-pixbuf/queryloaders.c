...
static void print_escaped(GString* contents, const char* str) {
    gchar* tmp = g_strescape(str, "");
    g_string_append_printf(contents, "\"%s\" ", tmp);
    g_free(tmp);
}

static int loader_sanity_check(const char* path, GdkPixbufFormat* info, GdkPixbufModule* vtable) {
...
static gchar* get_toplevel(void) {
...
static gchar* get_relative_path(const gchar* parent, const gchar* descendant) {
...
static void write_loader_info(GString* contents, const char* path, GdkPixbufFormat* info) {
    const GdkPixbufModulePattern* pattern;
    char** mime;
    char** ext;
...
static void query_module(GString* contents, const char* dir, const char* file) {
    char* path;
    GModule* module;
...
int main(int argc, char** argv) {
    ...
    if (g_ascii_strncasecmp(pixbuf_libdir, pixbuf_prefix, strlen(pixbuf_prefix)) == 0 &&
        ...
        *slash = '\0';
         ...
         */
        if (slash == NULL || g_ascii_strcasecmp(slash + 1, ".libs") == 0 ||
            ...
            libdir = g_build_filename(runtime_prefix, pixbuf_libdir + strlen(pixbuf_prefix) + 1, NULL);
        }
    }
    else {
        libdir = NULL;
    ...
    g_string_append_printf(contents,
                           ...
                           "#\n",
                           (prgname ? prgname : "gdk-pixbuf-query-loaders"), GDK_PIXBUF_VERSION);
...
#ifdef G_OS_WIN32
            infilename = g_locale_to_utf8(infilename, -1, NULL, NULL, NULL);
#endif
