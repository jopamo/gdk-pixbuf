...
static void gdk_pixbuf_scaled_anim_finalize(GObject* object) {
...
static gboolean is_static_image(GdkPixbufAnimation* anim) {
    GdkPixbufScaledAnim* scaled = (GdkPixbufScaledAnim*)anim;

    return gdk_pixbuf_animation_is_static_image(scaled->anim);
}

static GdkPixbuf* get_scaled_pixbuf(GdkPixbufScaledAnim* scaled, GdkPixbuf* pixbuf) {
    GQuark quark;
    gchar** options;
    ...
    scaled->current = gdk_pixbuf_scale_simple(
        pixbuf, MAX((int)((gdouble)gdk_pixbuf_get_width(pixbuf) * scaled->xscale + .5), 1),
        MAX((int)((gdouble)gdk_pixbuf_get_height(pixbuf) * scaled->yscale + .5), 1), GDK_INTERP_BILINEAR);

    /* Copy the original pixbuf options to the scaled pixbuf */
    if (options && scaled->current)
        g_object_set_qdata_full(G_OBJECT(scaled->current), quark, g_strdupv(options), (GDestroyNotify)g_strfreev);

    return scaled->current;
}

static GdkPixbuf* get_static_image(GdkPixbufAnimation* anim) {
    GdkPixbufScaledAnim* scaled = (GdkPixbufScaledAnim*)anim;
    GdkPixbuf* pixbuf;
...
static void get_size(GdkPixbufAnimation* anim, int* width, int* height) {
    GdkPixbufScaledAnim* scaled = (GdkPixbufScaledAnim*)anim;
...
static void gdk_pixbuf_scaled_anim_class_init(GdkPixbufScaledAnimClass* klass) {
...
static int get_delay_time(GdkPixbufAnimationIter* iter) {
...
static void gdk_pixbuf_scaled_anim_iter_finalize(GObject* object) {
...
static void gdk_pixbuf_scaled_anim_iter_class_init(GdkPixbufScaledAnimIterClass* klass) {
    GObjectClass* object_class;
