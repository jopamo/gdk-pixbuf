...
static void tiff_warning_handler(const char* mod, const char* fmt, va_list ap) {
    /* Don't print anything; we should not be dumping junk to
     * stderr, since that may be bad for some apps.
     */
}

static void tiff_set_handlers(void) {
...
static GdkPixbuf* tiff_image_parse(TIFF* tiff, TiffContext* context, GError** error) {
    guchar* pixels = NULL;
    ...
    pixbuf = gdk_pixbuf_new_from_data(pixels, GDK_COLORSPACE_RGB, TRUE, 8, width, height, rowstride, free_buffer, NULL);
    if (!pixbuf) {
        g_free(pixels);
        g_set_error_literal(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_INSUFFICIENT_MEMORY,
                            _("Insufficient memory to open TIFF file"));
...
static GdkPixbuf* gdk_pixbuf__tiff_image_load(FILE* f, GError** error) {
    TIFF* tiff = NULL;
    int fd;
    GdkPixbuf* pixbuf;
     ...
     */
    {
        ...
        if (DuplicateHandle(GetCurrentProcess(), (HANDLE)_get_osfhandle(fd), GetCurrentProcess(), &h, 0, FALSE,
    ...
    if (!tiff) {
        g_set_error_literal(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_CORRUPT_IMAGE, _("Failed to open TIFF image"));
        return NULL;
    }
...
static gpointer gdk_pixbuf__tiff_image_begin_load(GdkPixbufModuleSizeFunc size_func,
                                                  GdkPixbufModulePreparedFunc prepared_func,
                                                  GdkPixbufModuleUpdatedFunc updated_func,
                                                  gpointer user_data,
                                                  GError** error) {
...
static toff_t tiff_load_seek(thandle_t handle, toff_t offset, int whence) {
    TiffContext* context = (TiffContext*)handle;
...
static int tiff_load_close(thandle_t context) {
    return 0;
}
...
static int tiff_load_map_file(thandle_t handle, tdata_t* buf, toff_t* size) {
    TiffContext* context = (TiffContext*)handle;
...
static gboolean gdk_pixbuf__tiff_image_stop_load(gpointer data, GError** error) {
    ...
    tiff = TIFFClientOpen("libtiff-pixbuf", "r", data, tiff_load_read, tiff_load_write, tiff_load_seek, tiff_load_close,
                          tiff_load_size, tiff_load_map_file, tiff_load_unmap_file);
    if (!tiff) {
...
static gboolean make_available_at_least(TiffContext* context, guint needed) {
...
static gboolean gdk_pixbuf__tiff_image_load_increment(gpointer data, const guchar* buf, guint size, GError** error) {
...
static void copy_gray_row(gint* dest, guchar* src, gint width, gboolean has_alpha) {
    gint i;
    guchar* p;
...
static gboolean gdk_pixbuf__tiff_image_save_to_callback(GdkPixbufSaveFunc save_func,
                                                        gpointer user_data,
                                                        GdkPixbuf* pixbuf,
                                                        gchar** keys,
                                                        gchar** values,
    ...
    tiff = TIFFClientOpen("libtiff-pixbuf", "w", context, tiff_save_read, tiff_save_write, tiff_save_seek,
                          tiff_save_close, tiff_save_size, NULL, NULL);

    if (!tiff) {
        g_set_error_literal(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_FAILED, _("Failed to save TIFF image"));
        ...
        if (icc_profile != NULL) {
            ...
            if (icc_profile_size < 127) {
                g_set_error(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_BAD_OPTION,
                            _("Color profile has invalid length %d."), (gint)icc_profile_size);
                retval = FALSE;
    ...
    if (y < height) {
        g_set_error_literal(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_FAILED, _("Failed to write TIFF data"));
        TIFFClose(tiff);
        retval = FALSE;
        goto cleanup;
    }
    ...
    if (x_dpi != NULL && y_dpi != NULL) {
        ...
        if (y_dpi_value <= 0) {
            g_set_error(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_BAD_OPTION,
                        _("TIFF y-dpi must be greater than zero; value “%s” is not allowed."), y_dpi);
            retval = FALSE;
            goto cleanup;
        }
...
static gboolean save_to_file_cb(const gchar* buf, gsize count, GError** error, gpointer data) {
    ...
    if (count) {
        g_set_error_literal(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_FAILED, _("Couldn’t write to TIFF file"));
        return FALSE;
    }
...
static gboolean gdk_pixbuf__tiff_image_save(FILE* f, GdkPixbuf* pixbuf, gchar** keys, gchar** values, GError** error) {
    return gdk_pixbuf__tiff_image_save_to_callback(save_to_file_cb, f, pixbuf, keys, values, error);
}

static gboolean gdk_pixbuf__tiff_is_save_option_supported(const gchar* option_key) {
    if (g_strcmp0(option_key, "bits-per-sample") == 0 || g_strcmp0(option_key, "compression") == 0 ||
        g_strcmp0(option_key, "icc-profile") == 0 || g_strcmp0(option_key, "x-dpi") == 0 ||
...
MODULE_ENTRY(fill_info)(GdkPixbufFormat* info) {
    static const GdkPixbufModulePattern signature[] = {
        {"MM \x2a", "  z ", 100}, {"II\x2a ", "   z", 100}, {"II* \020   CR\002 ", "   z zzz   z", 0}, {NULL, NULL, 0}};
