...
static TGAColormap* colormap_new(guint n_colors) {
    TGAColormap* cmap;
...
static const TGAColor* colormap_get_color(TGAColormap* cmap, guint id) {
...
static void colormap_set_color(TGAColormap* cmap, guint id, const TGAColor* color) {
    if (id >= cmap->n_colors)
        return;
...
static void colormap_free(TGAColormap* cmap) {
    g_free(cmap);
}

static gboolean tga_skip_rest_of_image(TGAContext* ctx, GError** err) {
    gdk_pixbuf_buffer_queue_flush(ctx->input, gdk_pixbuf_buffer_queue_get_size(ctx->input));

    return TRUE;
}

static inline void tga_write_pixel(TGAContext* ctx, const TGAColor* color) {
...
static gsize tga_pixels_remaining(TGAContext* ctx) {
    gint width = gdk_pixbuf_get_width(ctx->pbuf);
    gint height = gdk_pixbuf_get_height(ctx->pbuf);
...
static gboolean tga_all_pixels_written(TGAContext* ctx) {
    gint height = gdk_pixbuf_get_height(ctx->pbuf);

    return ctx->pbuf_y >= height;
}

static void tga_emit_update(TGAContext* ctx) {
    gint width = gdk_pixbuf_get_width(ctx->pbuf);
    gint height = gdk_pixbuf_get_height(ctx->pbuf);
    ...
    if (ctx->hdr->flags & TGA_ORIGIN_UPPER)
        (*ctx->updated_func)(ctx->pbuf, 0, ctx->pbuf_y_notified, width, ctx->pbuf_y - ctx->pbuf_y_notified,
                             ctx->user_data);
    else
        (*ctx->updated_func)(ctx->pbuf, 0, height - ctx->pbuf_y, width, ctx->pbuf_y - ctx->pbuf_y_notified,
                             ctx->user_data);

    ctx->pbuf_y_notified = ctx->pbuf_y;
}

static gboolean tga_format_supported(guint type, guint bits_per_pixel) {
    switch (type) {
        ...
        case TGA_TYPE_RLE_GRAYSCALE:
            return bits_per_pixel == 8 || bits_per_pixel == 16;
...
static inline void tga_read_pixel(TGAContext* ctx, const guchar* data, TGAColor* color) {
    switch (ctx->hdr->type) {
        case TGA_TYPE_PSEUDOCOLOR:
        case TGA_TYPE_RLE_PSEUDOCOLOR:
...
static gboolean fill_in_context(TGAContext* ctx, GError** err) {
    ...
    ctx->cmap_size = ((ctx->hdr->cmap_bpp + 7) >> 3) * LE16(ctx->hdr->cmap_n_colors);
    ctx->cmap = colormap_new(LE16(ctx->hdr->cmap_n_colors));
    if (!ctx->cmap) {
        g_set_error_literal(err, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_INSUFFICIENT_MEMORY, _("Cannot allocate colormap"));
        return FALSE;
    }

    alpha = ((ctx->hdr->bpp == 16) || (ctx->hdr->bpp == 32) || (ctx->hdr->has_cmap && (ctx->hdr->cmap_bpp == 32)));
...
static gboolean tga_load_image(TGAContext* ctx, GError** err) {
    TGAColor color;
    GBytes* bytes;
...
static gboolean tga_load_rle_image(TGAContext* ctx, GError** err) {
    GBytes* bytes;
    TGAColor color;
...
static gboolean tga_load_colormap(TGAContext* ctx, GError** err) {
    GBytes* bytes;
    TGAColor color;
    const guchar* p;
    guint i, n_colors;
    ...
    if (ctx->hdr->has_cmap) {
        ...
        if ((ctx->hdr->type == TGA_TYPE_PSEUDOCOLOR) || (ctx->hdr->type == TGA_TYPE_RLE_PSEUDOCOLOR)) {
            g_set_error_literal(err, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_CORRUPT_IMAGE,
    ...
    if ((ctx->hdr->type == TGA_TYPE_RLE_PSEUDOCOLOR) || (ctx->hdr->type == TGA_TYPE_RLE_TRUECOLOR) ||
...
static gboolean tga_read_info(TGAContext* ctx, GError** err) {
...
static gboolean tga_load_header(TGAContext* ctx, GError** err) {
    GBytes* bytes;
    ...
    g_print(
        ...
        "flags %#x",
        ctx->hdr->infolen, ctx->hdr->has_cmap, ctx->hdr->type, LE16(ctx->hdr->cmap_start),
        LE16(ctx->hdr->cmap_n_colors), ctx->hdr->cmap_bpp, LE16(ctx->hdr->x_origin), LE16(ctx->hdr->y_origin),
        LE16(ctx->hdr->width), LE16(ctx->hdr->height), ctx->hdr->bpp, ctx->hdr->flags);
#endif
    if (LE16(ctx->hdr->width) == 0 || LE16(ctx->hdr->height) == 0) {
        g_set_error_literal(err, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_CORRUPT_IMAGE,
                            _("TGA image has invalid dimensions"));
        return FALSE;
    }
    if ((ctx->hdr->flags & TGA_INTERLEAVE_MASK) != TGA_INTERLEAVE_NONE) {
        g_set_error_literal(err, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_UNKNOWN_TYPE, _("TGA image type not supported"));
        return FALSE;
    }
...
static gboolean gdk_pixbuf__tga_load_increment(gpointer data, const guchar* buffer, guint size, GError** err) {
...
MODULE_ENTRY(fill_info)(GdkPixbufFormat* info) {
    static const GdkPixbufModulePattern signature[] = {{" \x1\x1", "x  ", 100}, {" \x1\x9", "x  ", 100},
                                                       {"  \x2", "xz ", 99}, /* only 99 since .CUR also matches this */
                                                       {"  \x3", "xz ", 100},   {"  \xa", "xz ", 100},
                                                       {"  \xb", "xz ", 100},   {NULL, NULL, 0}};
    static const gchar* mime_types[] = {"image/x-tga", NULL};
    static const gchar* extensions[] = {"tga", "targa", NULL};
