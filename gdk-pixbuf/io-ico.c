...
static void DumpBIH(unsigned char* BIH) {
    ...
    printf("biHeight    = %i \n", (int)(BIH[11] << 24) + (BIH[10] << 16) + (BIH[9] << 8) + (BIH[8]));
    printf("biPlanes    = %i \n", (int)(BIH[13] << 8) + (BIH[12]));
    printf("biBitCount  = %i \n", (int)(BIH[15] << 8) + (BIH[14]));
    printf("biCompress  = %i \n", (int)(BIH[19] << 24) + (BIH[18] << 16) + (BIH[17] << 8) + (BIH[16]));
    printf("biSizeImage = %i \n", (int)(BIH[23] << 24) + (BIH[22] << 16) + (BIH[21] << 8) + (BIH[20]));
    printf("biXPels     = %i \n", (int)(BIH[27] << 24) + (BIH[26] << 16) + (BIH[25] << 8) + (BIH[24]));
    printf("biYPels     = %i \n", (int)(BIH[31] << 24) + (BIH[30] << 16) + (BIH[29] << 8) + (BIH[28]));
    printf("biClrUsed   = %i \n", (int)(BIH[35] << 24) + (BIH[34] << 16) + (BIH[33] << 8) + (BIH[32]));
    printf("biClrImprtnt= %i \n", (int)(BIH[39] << 24) + (BIH[38] << 16) + (BIH[37] << 8) + (BIH[36]));
}
#endif
...
static gboolean gdk_pixbuf__ico_image_load_increment(gpointer data, const guchar* buf, guint size, GError** error);

static void context_free(struct ico_progressive_state* context) {
...
static gint compare_direntry_scores(gconstpointer a, gconstpointer b) {
...
static void DecodeHeader(guchar* Data, gint Bytes, struct ico_progressive_state* State, GError** error) {
    ...
    if (((Data[1] << 8) + Data[0]) != 0) {
        g_set_error(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_CORRUPT_IMAGE, _("Invalid header in icon (%s)"),
                    "first word");
        return;
    }
    ...
    for (I = 0; I < IconCount; I++) {
        ...
        DEBUG(g_print("Image %d: %d x %d\n\tDepth: %d\n", I, width, height, depth);
              if (imgtype == 2) g_print("\tHotspot: %d x %d\n", x_hot, y_hot);
              else g_print("\tColor planes: %d\n\tBits per pixel: %d\n", x_hot, y_hot);
    ...
    for (l = State->entries; l != NULL; l = g_list_next(l)) {
        ...
        if ((BIH[16] != 0) || (BIH[17] != 0) || (BIH[18] != 0) || (BIH[19] != 0)) {
    ...
    if (l == NULL) {
        g_set_error_literal(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_CORRUPT_IMAGE,
                            got_broken_header ? _("Invalid header in icon") : _("Compressed icons are not supported"));
        return;
    }
    ...
    State->Header.width = (int)(BIH[7] << 24) + (BIH[6] << 16) + (BIH[5] << 8) + (BIH[4]);
    if (State->Header.width == 0)
        State->Header.width = 256;

    State->Header.height = (int)((BIH[11] << 24) + (BIH[10] << 16) + (BIH[9] << 8) + (BIH[8])) / 2;
    ...
    if (State->Header.width != entry->width || State->Header.height != entry->height) {
        g_set_error(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_CORRUPT_IMAGE, _("Invalid header in icon (%s)"),
                    "image size");
        return;
    }
    ...
    if (State->Type == 32)
        ...
        g_set_error_literal(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_CORRUPT_IMAGE, _("Unsupported icon type"));
        return;
    }
    ...
    if (State->pixbuf == NULL) {
        ...
        State->pixbuf = gdk_pixbuf_new(GDK_COLORSPACE_RGB, TRUE, 8, State->Header.width, State->Header.height);
        if (!State->pixbuf) {
            g_set_error_literal(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_INSUFFICIENT_MEMORY,
                                _("Not enough memory to load icon"));
            return;
...
static void OneLine32(struct ico_progressive_state* context) {
    ...
    if (context->Header.Negative == 0)
        Pixels = (gdk_pixbuf_get_pixels(context->pixbuf) + rowstride * (context->Header.height - context->Lines - 1));
    else
        Pixels = (gdk_pixbuf_get_pixels(context->pixbuf) + rowstride * context->Lines);
...
static void OneLine16(struct ico_progressive_state* context) {
    int i;
    guchar* pixels;
    guchar* src;
    gsize rowstride = gdk_pixbuf_get_rowstride(context->pixbuf);
...
static void OneLine8(struct ico_progressive_state* context) {
    ...
    while (X < context->Header.width) {
        ...
        Pixels[X * 4 + 0] = context->HeaderBuf[4 * context->LineBuf[X] + INFOHEADER_SIZE + 2 + context->DIBoffset];
        Pixels[X * 4 + 1] = context->HeaderBuf[4 * context->LineBuf[X] + INFOHEADER_SIZE + 1 + context->DIBoffset];
        Pixels[X * 4 + 2] = context->HeaderBuf[4 * context->LineBuf[X] + INFOHEADER_SIZE + context->DIBoffset];
        Pixels[X * 4 + 3] = 0xff;
        X++;
    }
}
static void OneLine4(struct ico_progressive_state* context) {
    ...
    while (X < context->Header.width) {
        ...
        Pixels[X * 4 + 0] = context->HeaderBuf[4 * (Pix >> 4) + INFOHEADER_SIZE + 2 + context->DIBoffset];
        Pixels[X * 4 + 1] = context->HeaderBuf[4 * (Pix >> 4) + INFOHEADER_SIZE + 1 + context->DIBoffset];
        Pixels[X * 4 + 2] = context->HeaderBuf[4 * (Pix >> 4) + INFOHEADER_SIZE + context->DIBoffset];
        Pixels[X * 4 + 3] = 0xff;
        X++;
        if (X < context->Header.width) {
            /* Handle the other 4 bit pixel only when there is one */
            Pixels[X * 4 + 0] = context->HeaderBuf[4 * (Pix & 15) + INFOHEADER_SIZE + 2 + context->DIBoffset];
            Pixels[X * 4 + 1] = context->HeaderBuf[4 * (Pix & 15) + INFOHEADER_SIZE + 1 + context->DIBoffset];
            Pixels[X * 4 + 2] = context->HeaderBuf[4 * (Pix & 15) + INFOHEADER_SIZE + context->DIBoffset];
            Pixels[X * 4 + 3] = 0xff;
            X++;
        }
    }
...
static void OneLineTransp(struct ico_progressive_state* context) {
    ...
    if (context->Header.Negative == 0)
        Pixels =
            (gdk_pixbuf_get_pixels(context->pixbuf) + rowstride * (2 * context->Header.height - context->Lines - 1));
    else
        Pixels = (gdk_pixbuf_get_pixels(context->pixbuf) + rowstride * (context->Lines - context->Header.height));
    while (X < context->Header.width) {
        int Bit;
...
static void OneLine(struct ico_progressive_state* context) {
...

    {
        ...
        if (context->Header.Negative == 0 && context->Lines < context->Header.height)
            y = context->Header.height - y;
        (*context->updated_func)(context->pixbuf, 0, y, context->Header.width, 1, context->user_data);
    }
}
...
static gboolean gdk_pixbuf__ico_image_load_increment(gpointer data, const guchar* buf, guint size, GError** error) {
    ...
    while (size > 0) {
        ...
        if (context->HeaderDone < context->HeaderSize) { /* We still
            ...
            BytesToCopy = context->HeaderSize - context->HeaderDone;
            if (BytesToCopy > size)
                BytesToCopy = size;

            memmove(context->HeaderBuf + context->HeaderDone, buf, BytesToCopy);
            ...
            BytesToCopy = context->LineWidth - context->LineDone;
            if (BytesToCopy > size)
                BytesToCopy = size;
            ...
            if (BytesToCopy > 0) {
                ...
                memmove(context->LineBuf + context->LineDone, buf, BytesToCopy);
        ...
        if (context->HeaderDone >= 6 && context->pixbuf == NULL) {
            ...
            DecodeHeader(context->HeaderBuf, context->HeaderDone, context, &decode_err);
...
static gint write8(FILE* f, guint8* data, gint count) {
    gint bytes;
    gint written;
...
static gint write16(FILE* f, guint16* data, gint count) {
    gint i;
...
static gint write32(FILE* f, guint32* data, gint count) {
    gint i;
...
static gboolean fill_entry(IconEntry* icon, GdkPixbuf* pixbuf, gint hot_x, gint hot_y, GError** error) {
    ...
    switch (icon->depth) {
        ...
        case 16:
            ...
            break;
        default:
            g_set_error(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_BAD_OPTION, _("Unsupported depth for ICO file: %d"),
                        icon->depth);
            return FALSE;
    }
...
static void write_icon(FILE* f, GSList* entries) {
    IconEntry* icon;
    GSList* entry;
    guint8 bytes[4];
    guint16 words[4];
...
static gboolean ascii_strtoll(const gchar* str, gint64 minimum, gint64 maximum, gint64* out, GError** error) {
    gint64 retval;
    const gchar* end_ptr;
    ...
    if (errno != 0) {
        g_set_error_literal(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, g_strerror(errno));
        return FALSE;
    }
    else if (end_ptr == str || *end_ptr != '\0') {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT, "Argument is not an integer: %s", str);
        return FALSE;
    }
    else if ((maximum < G_MAXINT64 && retval > maximum) || (minimum > G_MININT64 && retval < minimum)) {
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_INVALID_ARGUMENT,
                    "Argument should be in range [%" G_GINT64_FORMAT ", %" G_GINT64_FORMAT "]: %s", minimum, maximum,
                    str);
        return FALSE;
    }
...
static gboolean gdk_pixbuf__ico_image_save(FILE* f, GdkPixbuf* pixbuf, gchar** keys, gchar** values, GError** error) {
    gint hot_x, hot_y;
    IconEntry* icon;
    GSList* entries = NULL;
    ...
    if (keys && *keys) {
        ...
        for (kiter = keys, viter = values; *kiter && *viter; kiter++, viter++) {
            ...
            if (strcmp(*kiter, "depth") == 0) {
                if (!ascii_strtoll(*viter, 1, 32, &out, error))
                    return FALSE;
                icon->depth = out;
            }
            else if (strcmp(*kiter, "x_hot") == 0) {
                if (!ascii_strtoll(*viter, G_MININT, G_MAXINT, &out, error))
                    return FALSE;
                hot_x = out;
            }
...
static gboolean gdk_pixbuf__ico_is_save_option_supported(const gchar* option_key) {
    if (g_strcmp0(option_key, "depth") == 0 || g_strcmp0(option_key, "x_hot") == 0 ||
...
MODULE_ENTRY(fill_info)(GdkPixbufFormat* info) {
    static const GdkPixbufModulePattern signature[] = {
        {"  \x1   ", "zz znz", 100}, {"  \x2   ", "zz znz", 100}, {NULL, NULL, 0}};
    static const gchar* mime_types[] = {"image/x-icon",
                                        "image/x-ico",
                                        "image/x-win-bitmap",
                                        ...
                                        "text/ico",
                                        NULL};
    static const gchar* extensions[] = {"ico", "cur", NULL};
