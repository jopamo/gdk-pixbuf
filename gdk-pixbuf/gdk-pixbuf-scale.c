...
void gdk_pixbuf_scale(const GdkPixbuf* src,
    ...
    _pixops_scale(dest_pixels, dest->width, dest->height, dest->rowstride, dest->n_channels, dest->has_alpha,
                  src_pixels, src->width, src->height, src->rowstride, src->n_channels, src->has_alpha, dest_x, dest_y,
                  dest_width, dest_height, offset_x, offset_y, scale_x, scale_y, (PixopsInterpType)interp_type);
}
...
void gdk_pixbuf_composite(const GdkPixbuf* src,
    ...
    _pixops_composite(dest_pixels, dest->width, dest->height, dest->rowstride, dest->n_channels, dest->has_alpha,
                      src_pixels, src->width, src->height, src->rowstride, src->n_channels, src->has_alpha, dest_x,
                      dest_y, dest_width, dest_height, offset_x, offset_y, scale_x, scale_y,
...
void gdk_pixbuf_composite_color(const GdkPixbuf* src,
    ...
    _pixops_composite_color(dest_pixels, dest_width, dest_height, dest->rowstride, dest->n_channels, dest->has_alpha,
                            src_pixels, src->width, src->height, src->rowstride, src->n_channels, src->has_alpha,
                            dest_x, dest_y, dest_width, dest_height, offset_x, offset_y, scale_x, scale_y,
                            (PixopsInterpType)interp_type, overall_alpha, check_x, check_y, check_size, color1, color2);
}
...
GdkPixbuf* gdk_pixbuf_scale_simple(const GdkPixbuf* src, int dest_width, int dest_height, GdkInterpType interp_type) {
    ...
    gdk_pixbuf_scale(src, dest, 0, 0, dest_width, dest_height, 0, 0, (double)dest_width / src->width,
                     (double)dest_height / src->height, interp_type);
...
GdkPixbuf* gdk_pixbuf_composite_color_simple(const GdkPixbuf* src,
    ...
    gdk_pixbuf_composite_color(src, dest, 0, 0, dest_width, dest_height, 0, 0, (double)dest_width / src->width,
                               (double)dest_height / src->height, interp_type, overall_alpha, 0, 0, check_size, color1,
                               color2);
...
GdkPixbuf* gdk_pixbuf_rotate_simple(const GdkPixbuf* src, GdkPixbufRotation angle) {
    ...
    switch (angle % 360) {
        case 0:
            ...
            break;
        case 90:
            dest = gdk_pixbuf_new(src->colorspace, src->has_alpha, src->bits_per_sample, src->height, src->width);
            if (!dest)
                return NULL;
            ...
            break;
        case 180:
            dest = gdk_pixbuf_new(src->colorspace, src->has_alpha, src->bits_per_sample, src->width, src->height);
            if (!dest)
                return NULL;
        ...
        default:
            dest = NULL;
            g_warning(
                "gdk_pixbuf_rotate_simple() can only rotate "
