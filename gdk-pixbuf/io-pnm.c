...
static gboolean gdk_pixbuf__pnm_image_load_increment(gpointer context, const guchar* buf, guint size, GError** error);
...
static gint pnm_skip_whitespace(PnmIOBuffer* inbuf, GError** error) {
...
static gint pnm_read_next_value(PnmIOBuffer* inbuf, gint max_length, guint* value, GError** error) {
    ...
    for (p = inptr, word = buf; (p < inend) && !g_ascii_isspace(*p) && (*p != '#') && (p - inptr < max_length);
         p++, word++)
        *word = *p;
    *word = '\0';
...
static gint pnm_read_header(PnmLoaderContext* context) {
    PnmIOBuffer* inbuf;
    gint retval;
    ...
    if (!context->width) {
        ...
        retval = pnm_read_next_value(inbuf, -1, &width, context->error);
    ...
    if (!context->height) {
        ...
        retval = pnm_read_next_value(inbuf, -1, &height, context->error);
    ...
    switch (context->type) {
        ...
        case PNM_FORMAT_PGM_RAW:
            if (!context->maxval) {
                retval = pnm_read_next_value(inbuf, -1, &context->maxval, context->error);
...
static gint pnm_read_raw_scanline(PnmLoaderContext* context) {
    PnmIOBuffer* inbuf;
...
static gint pnm_read_ascii_mono_scanline(PnmLoaderContext* context) {
    PnmIOBuffer* inbuf;
    guint value;
    gint retval;
    guchar* dptr;
...
static gint pnm_read_ascii_color_scanline(PnmLoaderContext* context) {
    PnmIOBuffer* inbuf;
    guint value, i;
    guchar* dptr;
    gint retval;
...
static gint pnm_read_scanline(PnmLoaderContext* context) {
    gint retval;
    ...
    switch (context->type) {
        ...
        case PNM_FORMAT_PPM:
            ...
            break;
        default:
            g_set_error_literal(context->error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_UNKNOWN_TYPE,
        ...
        if (!context.did_prescan) {
            switch (context.type) {
                ...
                case PNM_FORMAT_PPM_RAW:
                    ...
                    break;
                default:
                    retval = pnm_skip_whitespace(inbuf, context.error);
                    if (retval == PNM_FATAL_ERR)
                        return NULL;
                    else if (retval == PNM_SUSPEND)
                        continue;
 ...
 */
static gboolean gdk_pixbuf__pnm_image_stop_load(gpointer data, GError** error) {
 ...
 */
static gboolean gdk_pixbuf__pnm_image_load_increment(gpointer data, const guchar* buf, guint size, GError** error) {
    PnmLoaderContext* context = (PnmLoaderContext*)data;
    PnmIOBuffer* inbuf;
    const guchar* bufhd;
    ...
    while (TRUE) {
        ...
        while (context->output_row < context->height) {
            ...
            if (retval == PNM_SUSPEND) {
                ...
                /* send updated signal */
                (*context->updated_func)(context->pixbuf, 0, context->output_row - 1, context->width, 1,
                                         context->user_data);
            }
        }
...
MODULE_ENTRY(fill_info)(GdkPixbufFormat* info) {
    static const GdkPixbufModulePattern signature[] = {{"P1", NULL, 100}, {"P2", NULL, 100}, {"P3", NULL, 100},
                                                       {"P4", NULL, 100}, {"P5", NULL, 100}, {"P6", NULL, 100},
                                                       {NULL, NULL, 0}};
    static const gchar* mime_types[] = {"image/x-portable-anymap", "image/x-portable-bitmap",
                                        "image/x-portable-graymap", "image/x-portable-pixmap", NULL};
    static const gchar* extensions[] = {"pnm", "pbm", "pgm", "ppm", NULL};
