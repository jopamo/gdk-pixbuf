...
static void fatal_error_handler(j_common_ptr cinfo) {
    ...
    if (errmgr->error && *errmgr->error == NULL) {
        g_set_error(errmgr->error, GDK_PIXBUF_ERROR,
                    cinfo->err->msg_code == JERR_OUT_OF_MEMORY ? GDK_PIXBUF_ERROR_INSUFFICIENT_MEMORY
...
static void explode_gray_into_buf(struct jpeg_decompress_struct* cinfo, guchar** lines) {
    gint i, j;
    guint w;
...
static void stdio_init_source(j_decompress_ptr cinfo) {
    stdio_src_ptr src = (stdio_src_ptr)cinfo->src;
    src->start_of_file = FALSE;
}

static boolean stdio_fill_input_buffer(j_decompress_ptr cinfo) {
    stdio_src_ptr src = (stdio_src_ptr)cinfo->src;
    size_t nbytes;
...
static void stdio_skip_input_data(j_decompress_ptr cinfo, long num_bytes) {
    stdio_src_ptr src = (stdio_src_ptr)cinfo->src;
...
static void stdio_term_source(j_decompress_ptr cinfo) {}

static gchar* colorspace_name(const J_COLOR_SPACE jpeg_color_space) {
...
static gboolean jpeg_parse_exif_app2_segment(JpegExifContext* context, jpeg_saved_marker_ptr marker) {
    guint ret = FALSE;
...
static gboolean jpeg_parse_exif_app1(JpegExifContext* context, jpeg_saved_marker_ptr marker) {
    guint i;
    guint ret = FALSE;
    guint offset;
...
static void jpeg_parse_exif(JpegExifContext* context, j_decompress_ptr cinfo) {
...
static gchar* jpeg_get_comment(j_decompress_ptr cinfo) {
    jpeg_saved_marker_ptr cmarker;
...
static void jpeg_destroy_exif_context(JpegExifContext* context) {
    g_free(context->icc_profile);
}

/* Shared library entry point */
static GdkPixbuf* gdk_pixbuf__real_jpeg_image_load(FILE* f, struct jpeg_decompress_struct* cinfo, GError** error) {
    gint i;
    char otag_str[5];
    char* density_str;
    ...
    src->buffer =
        (JOCTET*)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_PERMANENT, JPEG_PROG_BUF_SIZE * sizeof(JOCTET));
    ...
    pixbuf = gdk_pixbuf_new(GDK_COLORSPACE_RGB, cinfo->out_color_components == 4 ? TRUE : FALSE, 8, cinfo->output_width,
                            cinfo->output_height);
 ...
 */
static gboolean gdk_pixbuf__jpeg_image_stop_load(gpointer data, GError** error) {
    ...
    if (!sigsetjmp(context->jerr.setjmp_buffer, 1)) {
        ...
        if (context->pixbuf && cinfo->output_scanline < cinfo->output_height) {
            my_src_ptr src = (my_src_ptr)cinfo->src;
...
static gboolean gdk_pixbuf__jpeg_image_load_lines(JpegProgContext* context, GError** error) {
    ...
    while (cinfo->output_scanline < cinfo->output_height) {
        ...
        /* send updated signal */
        (*context->updated_func)(context->pixbuf, 0, cinfo->output_scanline - 1, cinfo->image_width, nlines,
                                 context->user_data);
    }
 ...
 */
static gboolean gdk_pixbuf__jpeg_image_load_increment(gpointer data, const guchar* buf, guint size, GError** error) {
    JpegProgContext* context = (JpegProgContext*)data;
    struct jpeg_decompress_struct* cinfo;
    my_src_ptr src;
        ...
        if (!context->got_header) {
            ...
            if (cinfo->output_components == 3) {
                ...
                g_set_error(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_CORRUPT_IMAGE,
                            _("Unsupported number of color components (%d)"), cinfo->output_components);
                retval = FALSE;
                goto out;
            }

            context->pixbuf =
                gdk_pixbuf_new(GDK_COLORSPACE_RGB, has_alpha, 8, cinfo->output_width, cinfo->output_height);
            ...
            if (exif_context.icc_profile != NULL) {
                icc_profile_base64 =
                    g_base64_encode((const guchar*)exif_context.icc_profile, exif_context.icc_profile_size);
             ...
             */
            if (!gdk_pixbuf__jpeg_image_load_lines(context, error)) {
                retval = FALSE;
                goto out;
            }
...
static void to_callback_do_write(j_compress_ptr cinfo, gsize length) {
    ...
    destmgr = (ToFunctionDestinationManager*)cinfo->dest;
    if (!destmgr->save_func((gchar*)destmgr->buffer, length, destmgr->error, destmgr->user_data)) {
        struct error_handler_data* errmgr;
         ...
         */
        if (errmgr->error && *errmgr->error == NULL) {
            g_set_error_literal(errmgr->error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_CORRUPT_IMAGE,
...
static boolean to_callback_empty_output_buffer(j_compress_ptr cinfo) {
    ToFunctionDestinationManager* destmgr;
...
static gboolean real_save_jpeg(GdkPixbuf* pixbuf,
                               gchar** keys,
                               gchar** values,
                               GError** error,
    ...
    if (keys && *keys) {
        ...
        while (*kiter) {
            if (strcmp(*kiter, "quality") == 0) {
                ...
                if (x_density <= 0 || x_density > 65535) {
                    /* This is a user-visible error;
                     * lets people skip the range-checking
                     * in their app.
                     */
                ...
                if (y_density <= 0 || y_density > 65535) {
                    /* This is a user-visible error;
                     * lets people skip the range-checking
                     * in their app.
                     */
                ...
                if (icc_profile_size < 127) {
                    /* This is a user-visible error */
                    g_set_error(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_BAD_OPTION,
                                _("Color profile has invalid length “%u”."), (guint)icc_profile_size);
                    retval = FALSE;
                    goto cleanup;
                }
            }
...
static gboolean gdk_pixbuf__jpeg_image_save(FILE* f, GdkPixbuf* pixbuf, gchar** keys, gchar** values, GError** error) {
    return real_save_jpeg(pixbuf, keys, values, error, FALSE, f, NULL, NULL);
}

static gboolean gdk_pixbuf__jpeg_image_save_to_callback(GdkPixbufSaveFunc save_func,
                                                        gpointer user_data,
                                                        GdkPixbuf* pixbuf,
                                                        gchar** keys,
                                                        gchar** values,
                                                        GError** error) {
    return real_save_jpeg(pixbuf, keys, values, error, TRUE, NULL, save_func, user_data);
}

static gboolean gdk_pixbuf__jpeg_is_save_option_supported(const gchar* option_key) {
    if (g_strcmp0(option_key, "quality") == 0 || g_strcmp0(option_key, "icc-profile") == 0)
        return TRUE;
...
MODULE_ENTRY(fill_info)(GdkPixbufFormat* info) {
    static const GdkPixbufModulePattern signature[] = {{"\xff\xd8", NULL, 100}, {NULL, NULL, 0}};
    static const gchar* mime_types[] = {"image/jpeg", NULL};
    static const gchar* extensions[] = {"jpeg", "jpe", "jpg", NULL};
