...
void gdk_pixbuf_copy_area(const GdkPixbuf* src_pixbuf,
    ...
    gdk_pixbuf_scale(src_pixbuf, dest_pixbuf, dest_x, dest_y, width, height, (double)(dest_x - src_x),
                     (double)(dest_y - src_y), 1.0, 1.0, GDK_INTERP_NEAREST);
}
...
void gdk_pixbuf_saturate_and_pixelate(const GdkPixbuf* src, GdkPixbuf* dest, gfloat saturation, gboolean pixelate) {
    ...
    if (saturation == 1.0 && !pixelate) {
        if (dest != src)
            gdk_pixbuf_copy_area(src, 0, 0, gdk_pixbuf_get_width(src), gdk_pixbuf_get_height(src), dest, 0, 0);
    }
    else {
        int i, j, t;
...
static const gchar* get_localedir(void) {
    gchar* temp;
...
const gchar* gdk_pixbuf_gettext(const gchar* msgid) {
