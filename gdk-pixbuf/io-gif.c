...
static gboolean gif_read(GifContext* context, guchar* buffer, size_t len) {
    gboolean retval;
    if (context->file) {
        ...
        if (!retval && ferror(context->file)) {
            ...
            g_set_error(context->error, G_FILE_ERROR, g_file_error_from_errno(save_errno), _("Failure reading GIF: %s"),
...
static void gif_set_get_colormap(GifContext* context) {
    context->global_colormap_size = 0;
    context->state = GIF_GET_COLORMAP;
}

static void gif_set_get_colormap2(GifContext* context) {
...
static int get_data_block(GifContext* context, unsigned char* buf, gint* empty_block) {
...
static int gif_get_lzw(GifContext* context) {
    if (context->frame == NULL) {
        ...
        rowstride =
            gdk_pixbuf_calculate_rowstride(GDK_COLORSPACE_RGB, TRUE, 8, context->frame_len, context->frame_height);
        if (rowstride < 0 || !g_uint64_checked_mul(&len, rowstride, context->frame_height) || len >= G_MAXINT) {
            g_set_error_literal(context->error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_INSUFFICIENT_MEMORY,
    ...
    while (TRUE) {
        ...
        if ((retval != 0 || empty_block) && context->animation->frames->next == NULL) {
            GdkPixbuf* pixbuf = gdk_pixbuf_animation_get_static_image(GDK_PIXBUF_ANIMATION(context->animation));
            if (pixbuf)
                (*context->updated_func)(pixbuf, 0, 0, context->frame->width, context->frame->height,
                                         context->user_data);
        }
...
static int gif_prepare_lzw(GifContext* context) {
    ...
    if (context->lzw_set_code_size >= 12) {
        g_set_error_literal(context->error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_CORRUPT_IMAGE,
 ...
 */
static gint gif_init(GifContext* context) {
    ...
    if ((strcmp(version, "87a") != 0) && (strcmp(version, "89a") != 0)) {
        ...
        g_set_error(context->error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_CORRUPT_IMAGE,
                    _("Version %s of the GIF file format is not supported"), escaped_version);
...
static void gif_set_get_frame_info(GifContext* context) {
    context->state = GIF_GET_FRAME_INFO;
}

static gint gif_get_frame_info(GifContext* context) {
    unsigned char buf[9];
    ...
    g_print(">width: %d height: %d xoffset: %d yoffset: %d disposal: %d delay: %d transparent: %d interlace: %d\n",
            context->frame_len, context->frame_height, context->x_offset, context->y_offset, context->disposal,
            context->delay_time, context->transparent_index, context->frame_interlace);
#endif
...
static gint gif_get_next_step(GifContext* context) {
    unsigned char c;
    while (TRUE) {
...
static gint gif_main_loop(GifContext* context) {
    gint retval = 0;
...
static GifContext* new_context(GdkPixbufModuleSizeFunc size_func,
                               GdkPixbufModulePreparedFunc prepared_func,
                               GdkPixbufModuleUpdatedFunc updated_func,
                               gpointer user_data) {
    GifContext* context;
...
static void noop_prepared_notify(GdkPixbuf* pixbuf, GdkPixbufAnimation* anim, gpointer user_data) {}

static void noop_updated_notify(GdkPixbuf* pixbuf, int x, int y, int width, int height, gpointer user_data) {}

static GifContext* new_context_without_callbacks(void) {
    return new_context(noop_size_notify, noop_prepared_notify, noop_updated_notify, NULL);
}

/* Shared library entry point */
static GdkPixbuf* gdk_pixbuf__gif_image_load(FILE* file, GError** error) {
    GifContext* context;
    GdkPixbuf* pixbuf;
    gint retval;
...
static gpointer gdk_pixbuf__gif_image_begin_load(GdkPixbufModuleSizeFunc size_func,
                                                 GdkPixbufModulePreparedFunc prepared_func,
                                                 GdkPixbufModuleUpdatedFunc updated_func,
                                                 gpointer user_data,
                                                 GError** error) {
...
static gboolean gdk_pixbuf__gif_image_stop_load(gpointer data, GError** error) {
    ...
    if (context->animation->frames == NULL) {
        ...
        g_set_error_literal(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_INCOMPLETE_ANIMATION,
...
static gboolean gdk_pixbuf__gif_image_load_increment(gpointer data, const guchar* buf, guint size, GError** error) {
    gint retval;
    GifContext* context = (GifContext*)data;
...
static GdkPixbufAnimation* gdk_pixbuf__gif_image_load_animation(FILE* file, GError** error) {
    GifContext* context;
...
MODULE_ENTRY(fill_info)(GdkPixbufFormat* info) {
    static const GdkPixbufModulePattern signature[] = {{"GIF8", NULL, 100}, {NULL, NULL, 0}};
    static const gchar* mime_types[] = {"image/gif", NULL};
    static const gchar* extensions[] = {"gif", NULL};
