...
#define MAKE_TAG(a, b, c, d) ((guint32)d << 24 | (guint32)c << 16 | (guint32)b << 8 | (guint32)a)
...
#define BYTES_LEFT(context) ((context)->n_bytes - ((context)->byte - (context)->buffer))

static void read_int8(AniLoaderContext* context, guchar* data, int count) {
...
static guint32 read_int32(AniLoaderContext* context) {
    guint32 result;
...
static void context_free(AniLoaderContext* context) {
    if (!context)
        return;
...
static void updated_callback(GdkPixbufLoader* loader, gint x, gint y, gint width, gint height, gpointer data) {
    AniLoaderContext* context = (AniLoaderContext*)data;
...
static gboolean ani_load_chunk(AniLoaderContext* context, GError** error) {
    int i;
    ...
    if (context->chunk_id == TAG_icon) {
        ...
        if (context->loader == NULL) {
            ...
            g_signal_connect(context->loader, "area_prepared", G_CALLBACK(prepared_callback), context);
            g_signal_connect(context->loader, "area_updated", G_CALLBACK(updated_callback), context);
        }
    ...
    if (context->chunk_id == TAG_anih) {
        ...
        g_print("HeaderSize \t%" G_GUINT32_FORMAT "\nNumFrames \t%" G_GUINT32_FORMAT "\nNumSteps \t%" G_GUINT32_FORMAT
                ...
                "\nIconFlag \t%d"
                "\n",
                context->HeaderSize, context->NumFrames, context->NumSteps, context->Width, context->Height,
                context->BitCount, context->NumPlanes, context->DisplayRate, (context->Flags & 0x2) != 0,
                (context->Flags & 0x1) != 0);
#endif
        if (!(context->Flags & 0x2))
            context->NumSteps = context->NumFrames;
        if (context->NumFrames == 0 || context->NumFrames >= 1024 || context->NumSteps == 0 ||
        ...
        if (!context->animation->pixbufs || !context->animation->delay || !context->animation->sequence) {
            g_set_error_literal(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_INSUFFICIENT_MEMORY,
                                _("Not enough memory to load animation"));
            return FALSE;
        }
...
#ifdef DEBUG_ANI
    {
        ...
        g_print("Loaded chunk with ID '%c%c%c%c' and length %" G_GUINT32_FORMAT "\n", ((char*)&dummy)[0],
                ((char*)&dummy)[1], ((char*)&dummy)[2], ((char*)&dummy)[3], context->chunk_size);
    }
#endif
...
static gboolean gdk_pixbuf__ani_image_load_increment(gpointer data, const guchar* buf, guint size, GError** error) {
    AniLoaderContext* context = (AniLoaderContext*)data;
    ...
    if (context->data_size == 0) {
        ...
        if (riff_id != TAG_RIFF || context->data_size == 0 || chunk_id != TAG_ACON) {
            g_set_error_literal(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_CORRUPT_IMAGE,
                                _("Invalid header in animation"));
            return FALSE;
        }
...
static gpointer gdk_pixbuf__ani_image_begin_load(GdkPixbufModuleSizeFunc size_func,
                                                 GdkPixbufModulePreparedFunc prepared_func,
                                                 GdkPixbufModuleUpdatedFunc updated_func,
                                                 gpointer user_data,
                                                 GError** error) {
...
static gboolean gdk_pixbuf__ani_image_stop_load(gpointer data, GError** error) {
    AniLoaderContext* context = (AniLoaderContext*)data;
    gboolean retval;
...
MODULE_ENTRY(fill_info)(GdkPixbufFormat* info) {
    static const GdkPixbufModulePattern signature[] = {{"RIFF    ACON", "    xxxx    ", 100}, {NULL, NULL, 0}};
    static const gchar* mime_types[] = {"application/x-navi-animation", NULL};
    static const gchar* extensions[] = {"ani", NULL};
