...
static gint format_check(GdkPixbufModule* module, guchar* buffer, int size) {
    int i, j;
    gchar m;
...
static GSList* get_file_formats(void) {
    G_LOCK(init_lock);
    if (file_formats == NULL || !file_formats_inited)
...
static gboolean scan_string(const char** pos, GString* out) {
...
static gboolean skip_space(const char** pos) {
    const char* p = *pos;
...
static gchar* build_module_path(const gchar* path) {
#ifdef GDK_PIXBUF_RELOCATABLE
...
static gchar* gdk_pixbuf_get_module_file(void) {
    gchar* result = g_strdup(g_getenv("GDK_PIXBUF_MODULE_FILE"));

    if (!result)
        result =
            g_build_filename(GDK_PIXBUF_LIBDIR, "gdk-pixbuf-2.0", GDK_PIXBUF_BINARY_VERSION, "loaders.cache", NULL);
...
static gboolean gdk_pixbuf_load_module_unlocked(GdkPixbufModule* image_module, GError** error);

static gboolean gdk_pixbuf_io_init_modules(const char* filename, GError** error) {
#ifdef USE_GMODULE
    GIOChannel* channel;
    gchar* line_buf;
    gsize term;
    ...
    while (!have_error && g_io_channel_read_line(channel, &line_buf, NULL, &term, NULL) == G_IO_STATUS_NORMAL) {
        ...
        if (!module) {
            ...
            while (scan_string(&p, tmp_buf)) {
                if (tmp_buf->str[0] != 0) {
                    module->info->mime_types = g_realloc(module->info->mime_types, (n + 1) * sizeof(gchar*));
            ...
            while (scan_string(&p, tmp_buf)) {
                if (tmp_buf->str[0] != 0) {
                    module->info->extensions = g_realloc(module->info->extensions, (n + 1) * sizeof(gchar*));
            ...
            n_patterns++;
            module->info->signature = (GdkPixbufModulePattern*)g_realloc(
    ...
    if (g_slist_length(file_formats) <= num_formats) {
        char* filename_utf8 = g_filename_display_name(filename);
        g_set_error(error, G_IO_ERROR, G_IO_ERROR_NOT_INITIALIZED, "No new GdkPixbufModule loaded from '%s'",
                    filename_utf8);
        g_free(filename_utf8);
        return FALSE;
    }
...
static void gdk_pixbuf_io_init_builtin(void) {
...
#define try_module(format, id)                                                  \
    if (fill_info == NULL && strcmp(image_module->module_name, #format) == 0) { \
...
#ifdef USE_GMODULE
    {
        ...
        if (!module) {
            ...
            g_set_error(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_FAILED,
                        _("Unable to load image-loading module: %s: %s"), path_utf8, g_module_error());
            g_free(path_utf8);
            return FALSE;
        }
        ...
        if (g_module_symbol(module, "fill_vtable", &sym)) {
            ...
            char* path_utf8 = g_filename_display_name(path);
            g_set_error(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_FAILED,
                        _("Image-loading module %s does not export the proper interface; perhaps it’s from a different "
                          "gdk-pixbuf version?"),
                        path_utf8);
            g_free(path_utf8);
            return FALSE;
...
#else
        g_set_error(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_UNKNOWN_TYPE, _("Image type “%s” is not supported"),
                    image_module->module_name);
    return FALSE;
#endif /* !USE_GMODULE */
}

gboolean _gdk_pixbuf_load_module(GdkPixbufModule* image_module, GError** error) {
    gboolean ret;
...
GdkPixbufModule* _gdk_pixbuf_get_named_module(const char* name, GError** error) {
    GSList* modules;
    ...
    g_set_error(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_UNKNOWN_TYPE, _("Image type “%s” is not supported"), name);

    return NULL;
}

GdkPixbufModule* _gdk_pixbuf_get_module(guchar* buffer, guint size, const gchar* filename, GError** error) {
    GSList* modules;
    ...
    if ((uncertain || g_str_equal(mime_type, "text/plain") || g_str_equal(mime_type, "application/gzip")) &&
        filename != NULL) {
        g_free(mime_type);
    ...
    for (modules = get_file_formats(); modules; modules = g_slist_next(modules)) {
        ...
        if (strcmp(info->name, "GdkPixdata") == 0 && format_check(module, buffer, size) == 100) {
            selected = module;
            break;
        }
    }
    ...
    if (filename) {
        ...
        g_set_error(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_UNKNOWN_TYPE,
                    _("Couldn’t recognize the image file format for file “%s”"), display_name);
        g_free(display_name);
    }
    else
...
static GdkPixbufModule* _gdk_pixbuf_get_module_for_file(FILE* f, const gchar* filename, GError** error) {
    ...
    if (size == 0) {
        gchar* display_name;
        display_name = g_filename_display_name(filename);
        g_set_error(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_CORRUPT_IMAGE, _("Image file “%s” contains no data"),
                    display_name);
        g_free(display_name);
        return NULL;
    }
...
static void noop_size_notify(gint* width, gint* height, gpointer data) {}

static void prepared_notify(GdkPixbuf* pixbuf, GdkPixbufAnimation* anim, gpointer user_data) {
    if (pixbuf != NULL)
        g_object_ref(pixbuf);
    *((GdkPixbuf**)user_data) = pixbuf;
}

static void noop_updated_notify(GdkPixbuf* pixbuf, int x, int y, int width, int height, gpointer user_data) {}

static GdkPixbuf* generic_load_incrementally(GdkPixbufModule* module, FILE* f, GError** error) {
...
GdkPixbuf* _gdk_pixbuf_generic_image_load(GdkPixbufModule* module, FILE* f, GError** error) {
    GdkPixbuf* pixbuf = NULL;
...
GdkPixbuf* gdk_pixbuf_new_from_file(const char* filename, GError** error) {
    ...
    if (!f) {
        ...
        gchar* display_name;
        display_name = g_filename_display_name(filename);
        g_set_error(error, G_FILE_ERROR, g_file_error_from_errno(save_errno), _("Failed to open file “%s”: %s"),
    ...
    if (pixbuf == NULL && error != NULL && *error == NULL) {
        ...
        g_set_error(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_FAILED,
                    _("Failed to load image “%s”: reason not known, probably a corrupt image file"), display_name);
        g_free(display_name);
    }
        ...
        display_name = g_filename_display_name(filename);
        old = e->message;
        e->message = g_strdup_printf(_("Failed to load image “%s”: %s"), display_name, old);
        g_free(old);
        g_free(display_name);
    }
...
GdkPixbuf* gdk_pixbuf_new_from_file_at_size(const char* filename, int width, int height, GError** error) {
    return gdk_pixbuf_new_from_file_at_scale(filename, width, height, TRUE, error);
}
...
static void at_scale_data_async_data_free(AtScaleData* data) {
...
static void at_scale_size_prepared_cb(gpointer loader, int width, int height, gpointer data) {
    ...
    if (info->preserve_aspect_ratio && (info->width > 0 || info->height > 0)) {
        if (info->width < 0) {
        ...
        else if ((double)height * (double)info->width > (double)width * (double)info->height) {
...
GdkPixbuf* gdk_pixbuf_new_from_file_at_scale(const char* filename,
    ...
    g_signal_connect(loader, "size-prepared", G_CALLBACK(at_scale_size_prepared_cb), &info);
...
static GdkPixbuf* load_from_stream(GdkPixbufLoader* loader,
                                   GInputStream* stream,
...
static void load_from_stream_async_cb(GObject* stream, GAsyncResult* res, gpointer data) {
    GTask* task = data;
    GdkPixbufLoader* loader;
    GdkPixbuf* pixbuf;
    GError* error = NULL;
    ...
    if (bytes == NULL) {
        ...
        if (!gdk_pixbuf_loader_write(loader, g_bytes_get_data(bytes, NULL), g_bytes_get_size(bytes), &error)) {
            gdk_pixbuf_loader_close(loader, NULL);
            g_task_return_error(task, error);
            goto out;
        }
        g_input_stream_read_bytes_async(G_INPUT_STREAM(stream), LOAD_BUFFER_SIZE, G_PRIORITY_DEFAULT,
                                        g_task_get_cancellable(task), load_from_stream_async_cb, g_object_ref(task));
    }
    else {
        if (!gdk_pixbuf_loader_close(loader, &error)) {
            g_task_return_error(task, error);
...
void gdk_pixbuf_new_from_stream_at_scale_async(GInputStream* stream,
    ...
    loader = gdk_pixbuf_loader_new();
    g_signal_connect(loader, "size-prepared", G_CALLBACK(at_scale_size_prepared_cb), data);
    g_object_set_data_full(G_OBJECT(loader), "gdk-pixbuf-please-kill-me-later", data,
    ...
    g_input_stream_read_bytes_async(stream, LOAD_BUFFER_SIZE, G_PRIORITY_DEFAULT, cancellable,
                                    load_from_stream_async_cb, task);
}
...
GdkPixbuf* _gdk_pixbuf_new_from_resource_try_pixdata(const char* resource_path) {
     ...
     */
    if (g_resources_get_info(resource_path, 0, &data_size, NULL, NULL) && data_size > sizeof(guint32) &&
 ...
 * Since: 2.26
 **/
GdkPixbuf* gdk_pixbuf_new_from_resource(const gchar* resource_path, GError** error) {
    GInputStream* stream;
    GdkPixbuf* pixbuf;
...
static void get_file_info_async_data_free(GetFileInfoAsyncData* data) {
    g_free(data->filename);
    g_slice_free(GetFileInfoAsyncData, data);
}

static void get_file_info_thread(GTask* task,
    ...
    if (format == NULL) {
        g_task_return_new_error(task, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_UNKNOWN_TYPE,
                                "Failed to recognize image format");
    }
    else {
        g_task_return_pointer(task, gdk_pixbuf_format_copy(format), (GDestroyNotify)gdk_pixbuf_format_free);
    }
}
...
static void collect_save_options(va_list opts, gchar*** keys, gchar*** vals) {
    gchar* key;
    gchar* val;
    gchar* next;
    gint count;
...
static gboolean save_to_file_callback(const gchar* buf, gsize count, GError** error, gpointer data) {
    ...
    if (n != count) {
        gint save_errno = errno;
        g_set_error(error, G_FILE_ERROR, g_file_error_from_errno(save_errno), _("Error writing to image file: %s"),
...
static gboolean gdk_pixbuf_real_save(GdkPixbuf* pixbuf,
                                     FILE* filehandle,
                                     const char* type,
                                     gchar** keys,
                                     gchar** values,
    ...
    if (image_module->save) {
        /* save normally */
        ret = (*image_module->save)(filehandle, pixbuf, keys, values, error);
    }
    else if (image_module->save_to_callback) {
        /* save with simple callback */
        ret = (*image_module->save_to_callback)(save_to_file_callback, filehandle, pixbuf, keys, values, error);
    }
    else {
        /* can't save */
...
static gboolean save_to_callback_with_tmp_file(GdkPixbufModule* image_module,
                                               GdkPixbuf* pixbuf,
...
static gboolean gdk_pixbuf_real_save_to_callback(GdkPixbuf* pixbuf,
                                                 GdkPixbufSaveFunc save_func,
                                                 gpointer user_data,
                                                 const char* type,
                                                 gchar** keys,
    ...
    if (image_module->save_to_callback) {
        /* save normally */
        ret = (*image_module->save_to_callback)(save_func, user_data, pixbuf, keys, values, error);
    }
    else if (image_module->save) {
        /* use a temporary file */
        ret = save_to_callback_with_tmp_file(image_module, pixbuf, save_func, user_data, keys, values, error);
    }
    else {
        /* can't save */
        g_set_error(error, GDK_PIXBUF_ERROR, GDK_PIXBUF_ERROR_UNSUPPORTED_OPERATION,
...
gboolean gdk_pixbuf_save(GdkPixbuf* pixbuf, const char* filename, const char* type, GError** error, ...) {
    ...
    result = gdk_pixbuf_savev(pixbuf, filename, type, keys, values, error);
...
gboolean gdk_pixbuf_savev(GdkPixbuf* pixbuf,
    ...
    if (f == NULL) {
        gint save_errno = errno;
        gchar* display_name = g_filename_display_name(filename);
        g_set_error(error, G_FILE_ERROR, g_file_error_from_errno(save_errno), _("Failed to open “%s” for writing: %s"),
                    display_name, g_strerror(save_errno));
        g_free(display_name);
        return FALSE;
    }

    result = gdk_pixbuf_real_save(pixbuf, f, type, option_keys, option_values, error);
    ...
    if (fclose(f) < 0) {
        ...
        g_set_error(error, G_FILE_ERROR, g_file_error_from_errno(save_errno),
                    _("Failed to close “%s” while writing image, all data may not have been saved: %s"), display_name,
                    g_strerror(save_errno));
        g_free(display_name);
        return FALSE;
    }
...
gboolean gdk_pixbuf_savev_utf8(GdkPixbuf* pixbuf,
                               ...
                               char** option_values,
                               GError** error) {
    return gdk_pixbuf_savev(pixbuf, filename, type, option_keys, option_values, error);
}
...
gboolean gdk_pixbuf_save_to_callback(GdkPixbuf* pixbuf,
    ...
    result = gdk_pixbuf_save_to_callbackv(pixbuf, save_func, user_data, type, keys, values, error);
...
gboolean gdk_pixbuf_save_to_callbackv(GdkPixbuf* pixbuf,
    ...
    result = gdk_pixbuf_real_save_to_callback(pixbuf, save_func, user_data, type, option_keys, option_values, error);
...
gboolean gdk_pixbuf_save_to_buffer(GdkPixbuf* pixbuf,
    ...
    result = gdk_pixbuf_save_to_bufferv(pixbuf, buffer, buffer_size, type, keys, values, error);
...
static gboolean save_to_buffer_callback(const gchar* data, gsize count, GError** error, gpointer user_data) {
...
gboolean gdk_pixbuf_save_to_bufferv(GdkPixbuf* pixbuf,
    ...
    *buffer_size = 0;
    ...
    if (!gdk_pixbuf_save_to_callbackv(pixbuf, save_to_buffer_callback, &sdata, type, option_keys, option_values,
                                      error)) {
        g_free(sdata.buffer);
        return FALSE;
    }
...
static gboolean save_to_stream(const gchar* buffer, gsize count, GError** error, gpointer data) {
    ...
    while (remaining > 0) {
        buffer += written;
        remaining -= written;
        written = g_output_stream_write(sdata->stream, buffer, remaining, sdata->cancellable, &my_error);
        if (written < 0) {
            if (!my_error) {
                g_set_error_literal(error, G_IO_ERROR, 0, _("Error writing to image stream"));
            }
            else {
...
gboolean gdk_pixbuf_save_to_streamv(GdkPixbuf* pixbuf,
    ...
    return gdk_pixbuf_save_to_callbackv(pixbuf, save_to_stream, &data, type, option_keys, option_values, error);
}
...
gboolean gdk_pixbuf_save_to_stream(GdkPixbuf* pixbuf,
    ...
    res = gdk_pixbuf_save_to_streamv(pixbuf, stream, type, keys, values, cancellable, error);
...
static void save_to_stream_async_data_free(SaveToStreamAsyncData* data) {
    if (data->stream)
...
static void save_to_stream_thread(GTask* task,
                                  GdkPixbuf* pixbuf,
    ...
    retval =
        gdk_pixbuf_save_to_callbackv(pixbuf, save_to_stream, &sync_data, data->type, data->keys, data->values, &error);
...
void gdk_pixbuf_save_to_stream_async(GdkPixbuf* pixbuf,
    ...
    gdk_pixbuf_save_to_streamv_async(pixbuf, stream, type, keys, values, cancellable, callback, user_data);
    g_strfreev(keys);
    g_strfreev(values);
}
...
GdkPixbufFormat* _gdk_pixbuf_get_format(GdkPixbufModule* module) {
...
G_DEFINE_BOXED_TYPE(GdkPixbufFormat, gdk_pixbuf_format, gdk_pixbuf_format_copy, gdk_pixbuf_format_free)
