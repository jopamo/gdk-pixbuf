...
static void gdip_set_error_from_hresult(GError** error, gint code, HRESULT hr, const char* format) {
    gchar* msg;
...
static void gdip_set_error_from_gpstatus(GError** error, gint code, GpStatus status) {
    const char* msg;
...
static gboolean gdip_init(void) {
...
static gboolean GetEncoderClsid(const WCHAR* format, CLSID* pClsid) {
    UINT num, size;
    int j;
...
static HGLOBAL gdip_buffer_to_hglobal(const gchar* buffer, size_t size, GError** error) {
    HGLOBAL hg = NULL;
...
static gboolean gdip_save_bitmap_to_callback(GpBitmap* bitmap,
                                             const CLSID* format,
...
static GpBitmap* gdip_pixbuf_to_bitmap(GdkPixbuf* pixbuf) {
...
static GpBitmap* gdip_buffer_to_bitmap(GdipContext* context, GError** error) {
    HRESULT hr;
    HGLOBAL hg = NULL;
    GpBitmap* bitmap = NULL;
    IStream* stream = NULL;
...
static GpImage* gdip_buffer_to_image(GdipContext* context, GError** error) {
    HRESULT hr;
    HGLOBAL hg = NULL;
    GpImage* image = NULL;
    IStream* stream = NULL;
...
static void gdip_bitmap_get_size(GpBitmap* bitmap, guint* width, guint* height) {
...
static void gdip_bitmap_get_has_alpha(GpBitmap* bitmap, gboolean* has_alpha) {
    guint flags = 0;
...
static gboolean gdip_bitmap_get_n_frames(GpBitmap* bitmap, guint* n_frames, gboolean timeDimension) {
    if (bitmap == NULL || n_frames == NULL)
        return FALSE;

    *n_frames = 1;

    return (Ok == GdipImageGetFrameCount((GpImage*)bitmap, (timeDimension ? &FrameDimensionTime : &FrameDimensionPage),
                                         n_frames));
}

static gboolean gdip_bitmap_select_frame(GpBitmap* bitmap, guint frame, gboolean timeDimension) {
    if (bitmap == NULL)
        return FALSE;

    return (Ok == GdipImageSelectActiveFrame((GpImage*)bitmap,
                                             (timeDimension ? &FrameDimensionTime : &FrameDimensionPage), frame));
}

static gboolean gdip_bitmap_get_property_as_string(GpBitmap* bitmap, guint propertyId, gchar** str) {
    guint item_size;
    gboolean success = FALSE;
...
static gboolean gdip_bitmap_get_frame_delay(GpBitmap* bitmap, guint frame, guint* delay) {
    ...
    *delay = 0;
    ...
    if (Ok == GdipGetPropertyItemSize((GpImage*)bitmap, PropertyTagFrameDelay, &item_size)) {
        ...
        if (Ok == GdipGetPropertyItem((GpImage*)bitmap, PropertyTagFrameDelay, item_size, item)) {
            ...
            /* PropertyTagFrameDelay. Time delay, in hundredths of a second, between two frames in an animated GIF
             * image. */
...
static gboolean gdip_bitmap_get_n_loops(GpBitmap* bitmap, guint* loops) {
    guint item_size;
    gboolean success = FALSE;
...
static void destroy_gdipcontext(GdipContext* context) {
    if (context != NULL) {
...
static void emit_updated(GdipContext* context, GdkPixbuf* pixbuf) {
    (*context->updated_func)(pixbuf, 0, 0, gdk_pixbuf_get_width(pixbuf), gdk_pixbuf_get_height(pixbuf),
                             context->user_data);
}

static void emit_prepared(GdipContext* context, GdkPixbuf* pixbuf, GdkPixbufAnimation* anim) {
    (*context->prepared_func)(pixbuf, anim, context->user_data);
}

static gpointer gdk_pixbuf__gdip_image_begin_load(GdkPixbufModuleSizeFunc size_func,
                                                  GdkPixbufModulePreparedFunc prepared_func,
                                                  GdkPixbufModuleUpdatedFunc updated_func,
                                                  gpointer user_data,
                                                  GError** error) {
...
static gboolean gdk_pixbuf__gdip_image_load_increment(gpointer data, const guchar* buf, guint size, GError** error) {
...
static GdkPixbuf* gdip_bitmap_to_pixbuf(GpBitmap* bitmap, GError** error) {
...
static gboolean stop_load(GpBitmap* bitmap, GdipContext* context, GError** error) {
    guint n_frames = 1, i;
...
static gboolean gdk_pixbuf__gdip_image_stop_load(gpointer data, GError** error) {
    GdipContext* context = (GdipContext*)data;
    GpBitmap* bitmap = NULL;
...
static gboolean gdk_pixbuf__gdip_image_stop_vector_load(gpointer data, GError** error) {
    GdipContext* context = (GdipContext*)data;
