...
static void gdk_pixbuf_gif_anim_get_size(GdkPixbufAnimation* anim, int* width, int* height);
G_GNUC_BEGIN_IGNORE_DEPRECATIONS
static GdkPixbufAnimationIter* gdk_pixbuf_gif_anim_get_iter(GdkPixbufAnimation* anim, const GTimeVal* start_time);
...
static void iter_clear(GdkPixbufGifAnimIter* iter) {
    iter->current_frame = NULL;
}

static void iter_restart(GdkPixbufGifAnimIter* iter) {
    iter_clear(iter);
...
G_GNUC_BEGIN_IGNORE_DEPRECATIONS
static gboolean gdk_pixbuf_gif_anim_iter_advance(GdkPixbufAnimationIter* anim_iter, const GTimeVal* current_time) {
    GdkPixbufGifAnimIter* iter;
    gint elapsed;
    gint loop;
    GList* tmp;
...
int gdk_pixbuf_gif_anim_iter_get_delay_time(GdkPixbufAnimationIter* anim_iter) {
    GdkPixbufFrame* frame;
    GdkPixbufGifAnimIter* iter;
...
static void composite_frame(GdkPixbufGifAnim* anim, GdkPixbufFrame* frame) {
    ...
    if (frame->action == GDK_PIXBUF_FRAME_REVERT) {
        ...
        if (anim->last_frame_revert_data != NULL)
            gdk_pixbuf_copy_area(anim->last_frame_data, frame->x_offset, frame->y_offset, frame->width, frame->height,
                                 anim->last_frame_revert_data, 0, 0);
    }
    ...
    n_indexes = lzw_decoder_feed(lzw_decoder, frame->lzw_data->data, frame->lzw_data->len, index_buffer,
...
GdkPixbuf* gdk_pixbuf_gif_anim_iter_get_pixbuf(GdkPixbufAnimationIter* anim_iter) {
    ...
    for (link = g_list_find(anim->frames, anim->last_frame); link->next != NULL && link->data != requested_frame;
         link = link->next) {
        ...
        switch (frame->action) {
            ...
            case GDK_PIXBUF_FRAME_REVERT:
                ...
                if (anim->last_frame_revert_data != NULL)
                    gdk_pixbuf_copy_area(anim->last_frame_revert_data, 0, 0, anim->last_frame->width,
                                         anim->last_frame->height, anim->last_frame_data, anim->last_frame->x_offset,
...
static gboolean gdk_pixbuf_gif_anim_iter_on_currently_loading_frame(GdkPixbufAnimationIter* anim_iter) {
    GdkPixbufGifAnimIter* iter;
